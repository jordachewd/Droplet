...
3. **Personas are plan-gated (three-tier access)** — **Full access:** Lite: Strategist, Developer (2). Pro: all Lite + Teacher, Creator, Wellness (5). Premium: all 6 personas. **Limited access (trial):** Non-full-access personas are available with reduced limits (5 prompts/conversation, 3 images, 2 audio, 1 video per 30-day window). Trial limits are global across all limited personas and tracked separately from plan limits. **Blocked:** Admin can override and fully block personas per plan via admin settings. Personas are the core product differentiator.
4. **Prices: Pro = $19, Premium = $39** — enforce everywhere.
5. **Lite limits: 5 conversations/day, 10 prompts/conversation, 3 image generations/month, 3 audio generations/month, 1 video generation/month.** All features limited, none blocked.
...
9. **App name is Droplet** — the rename from Cellesseon is complete. Legacy localStorage keys (`cellesseon-theme-mode`, `cellesseon-sidebar-collapsed`) must be removed and replaced with `droplet-` equivalents (Phase 172, owner override — deprecation cycle waived).
10. **All features (image, audio, video) available in all plans and all personas** — differentiated by plan limits (quantity) and persona purpose (prompt context). No feature is plan-exclusive or persona-exclusive.
11. **6 personas total** — Strategist (merged with Analyst capabilities), Teacher, Developer, Creator, Wellness, Interviewer. Each persona acts as an independent AI agent trained and skilled for its field. Must be pragmatic, direct, honest, grounded in reality, and practical. No evasions, no unnecessary language, no artificial questions. Verify information before delivering. Get right to the point. Displayed in 3-per-row grid on desktop. Each persona has a representative hero image. All persona configuration managed from admin panel by admin role only.
12. **Admin role = full permissions** — `admin` role users must have full access to all features and all personas with NO limitations. Admin bypasses all plan limits (conversations, prompts, media generations), all trial restrictions, and all persona gating. Enforced in backend (`/api/openai` route), not just UI components.
...
- **Durable usage counters** — daily/monthly usage limits MUST use durable counters that are incremented on creation and never decremented by deletion. Never derive usage counts from live document counts (`countDocuments`) that are affected by user-initiated deletion.
...
```
src/app/
  (public)/         — public marketing/legal pages (/, /about, /plans, /personas, /privacy, /cookies, /terms)
...
- **Destructive actions require confirmation** — any delete, remove, suspend, or irreversible action MUST show a confirmation dialog before executing. This applies to both admin and user-facing flows. Never fire destructive server actions from a plain `<form action>` without user acknowledgement.
- **All actions must show visual feedback** — every form submission (save, update, delete, create) must display a visual result using the app's `AlertMessage` component. Colors: green = success, red = error, orange = warning, blue = info. No silent reloads.
- **Long-running actions must show loading state** — use `useFormStatus()` or equivalent to show loading indicators (e.g., `LoadingBubbles`, disabled button with "Saving..." text) during form submission. Prevents double-submission and informs the user.
- **No `window.alert()` or `window.confirm()` in production UI** — use the app's design system components (`AlertMessage`, `ConfirmationModal`) instead of browser native dialogs. `ConfirmationModal` is live at `src/components/shared/confirmation-modal.tsx` (Phase 61.1). Zero `window.confirm()` and zero `window.alert()` in src/.

## Planned Integrations

- **Zod** — TypeScript-first schema validation. Dependencies installed. Use for all input validation, API request/response schemas, and form validation. Check Zod Skills and MCPs for best practices.
- **Zustand** — Lightweight state management. Dependencies installed. Use for client-side global state (chat state, UI state, user preferences). Check Zustand Skills and Context7 MCP for documentation.
