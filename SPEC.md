...
> Last updated: 2026-03-28 (PM audit #72). Milestones 0–25 COMPLETE. TDD rebuild COMPLETE (Phases 120.1–120.7). WCAG 2.2 AA COMPLETE. Phase 141 (suspended user enforcement) COMPLETE. Phase 149 (SSE heartbeat streaming fix) COMPLETE. Admin configurability ALL RESOLVED. User deletion cascade COMPLETE (Phase 150). Library uploaded tab COMPLETE (Phase 151). Payment checkout FULLY FIXED (Phase 152 redirect + Phase 157 webhook schema). Admin hydration fix COMPLETE (Phase 153). Suspended user UX COMPLETE (Phase 154). Scrollbar removal COMPLETE (Phase 155). Rate limiting on all routes COMPLETE (Phase 142). Server-only guards on all constants COMPLETE (Phase 156). **All 7 gates GREEN.** 586 unit tests (101 suites). E2E: 8 spec files. Coverage: 85/80/85/85. Zero `as never` casts. Lint: 0 errors, 0 warnings. Active issues: TD-STREAM-02 (HIGH — streaming catch/finally can throw on closed controller, Phase 158), TD-SCROLLBAR-02 (HIGH — 2 dangling `droplet-scrollbar` refs, Phase 155.1), TD-BTN-TEST (HIGH — button test asserts `btn-md` but component defaults `btn-sm`, Phase 159), TD-ENV-01 (MEDIUM — unsafe process.env access, Phase 143). Build passing. Node.js 24.12.0.
...
> Last updated: PM audit #72 (2026-03-28).

### Active — HIGH Priority

| ID              | Area | Description                                                                                                                                                                                                                           | Phase |
| --------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| TD-SCROLLBAR-02 | UX   | **HIGH.** 2 dangling `droplet-scrollbar` class references remain in `admin-layout-shell.tsx` L86 and L117 despite Phase 155 claiming zero refs. CSS class already removed so no functional impact, but contradicts Phase 155 claim.   | 155.1 |
| TD-STREAM-02    | SSE  | **HIGH.** Streaming catch block calls `writeStreamEvent` on potentially closed controller. `controller.close()` in finally also throws if already closed. Client shows "stream ended unexpectedly" on infrastructure/server failures. | 158   |
| TD-BTN-TEST     | Test | **HIGH.** Button test asserts `btn-md` but component defaults to `btn-sm`. Test is wrong, not the component. Blocks full gate green.                                                                                                  | 159   |
...
| TD-SERVERONLY-01 | Security | ~~4 constants files missing `server-only` guards.~~ **RESOLVED (Phase 156).** All 4 files (`plans.tsx`, `assistant-personas.tsx`, `faqs.tsx`, `stop-reasons.ts`) have `import "server-only"`.                                        | 156   |
| TD-RATELIMIT-02  | Security | ~~No rate limiting on `/api/upload`, `/api/aws`, `/api/download`.~~ **RESOLVED (Phase 142).** All 3 routes have `enforceSlidingWindowRateLimit` — upload: 30/60s, aws: 30/60s, download: 60/60s.                                     | 142   |
| TD-CHECKOUT-01   | Billing  | ~~`redirect()` inside try/catch in `checkoutPlan()`.~~ **RESOLVED (Phase 152).** `redirectUrl` variable declared outside try/catch. Redirect executes correctly. TD-WEBHOOK-01 (exposed subsequently) also now RESOLVED (Phase 157). | 152   |
