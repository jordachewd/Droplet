...
> Last updated: 2026-03-23 (PM audit #50. All Phases 1–113.2 complete. Milestone 24 COMPLETE. Milestone 25 IN PROGRESS — TDD testing rebuild + admin configurability closure. TD-SEC-09 RESOLVED (Phase 105). TD-SEC-10 RESOLVED (Phase 113.1). TD-SEC-11 RESOLVED (Phase 113.2). TD-KNIP-01 RESOLVED (Phase 111.1). Active: TD-HARDCODE-01 (HIGH — stop reason messages, Phase 107), TD-REUSE-04 (MEDIUM — ChatApiResponse duplication, Phase 106), TD-WCAG-05 (MEDIUM — tabs arrow-key, Phase 108), TD-WCAG-06 (HIGH — decorative icons, Phase 109), TD-WCAG-07 (MEDIUM — AvatarMenu keyboard, Phase 114). 443 unit tests (74 suites). E2E: 108 passed, 0 failed, 25 skipped. **All 7 gates GREEN.** Coverage: 78.55/66.3/83.53/78.85 vs 76/65/79/76 (MET). Build passing. Node.js 24.12.0 runtime.)
...
- **Coverage**: v8 provider, thresholds: 76% statements / 65% branches / 79% functions / 76% lines. Actual: 78.55/66.3/83.53/78.85. Gate PASSES. Reporters: text, json-summary, lcov. Setup file: `tests/unit/vitest.setup.ts` (global mock cleanup). Phase 98 targets raising thresholds to 82/78/82/82+.
...
- **Gaps**: No E2E for Stripe checkout flow. No admin action behavioral tests. No E2E for user deletion cascade. Branch coverage 66.3% is lowest metric — worst files: deleteFileFromAWS.tsx (0%), chat-body.tsx (14%), mongoose.tsx (21.73%), admin-queries.ts (20.14%), admin.actions.tsx (27.81%). E2E: 108 passed, 0 failed, 25 skipped — all GREEN. WCAG axe-core violations all fixed (Phase 103.1–103.4). **All 7 gates GREEN.** Phase 112.1 test audit COMPLETE — 51 KEEP, 19 REFACTOR, 2 REBUILD, 9 missing test suites identified. TDD rebuild (Phase 112.2+) is primary work directive. Phase 112.2 started: effective-persona-access.ts (100% branch ✅), handleError.tsx (100% branch ✅). TD-SEC-10 RESOLVED (Phase 113.1). TD-SEC-11 RESOLVED (Phase 113.2).
...
| ID             | Area     | Description                                                                                                                                                                                                                                      | Severity          |
| -------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| TD-SEC-09      | Security | ~~`classify-task-complexity.ts` and `message-policy.ts` are server-only utilities consumed only by `/api/openai` route but lack `import "server-only"` guard.~~ **RESOLVED (Phase 105).** Guards added to both files.                            | ~~HIGH~~ Resolved |
...
| TD-WCAG-05     | A11y     | Library tabs (`library-tabs.tsx`) and admin settings tabs (`admin-settings-tabs.tsx`) have `role="tablist"` + `aria-selected` but no arrow-key navigation handler. WCAG tablist pattern requires Left/Right arrow key focus movement. Phase 108. | MEDIUM            |
| TD-WCAG-06     | A11y     | ~20 components have decorative `<i className="bi bi-*">` icons missing `aria-hidden="true"`. Screen readers announce empty content. Architect audit confirmed full list. Phase 109.                                                              | HIGH              |

### Active — CRITICAL/HIGH Priority (PM Audit #49, Triple-Audit — New Findings)

| ID         | Area     | Description                                                                                                                                                                                                                     | Severity              |
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| TD-SEC-10  | Security | ~~`createTaskSchema` in `task.actions.tsx` uses `.passthrough()` instead of `.strict()`.~~ **RESOLVED (Phase 113.1).** Changed to `.strict()`. Arbitrary field injection prevented. Unit tests updated.                         | ~~CRITICAL~~ Resolved |
| TD-SEC-11  | Security | ~~`checkout-success` page at `src/app/(public)/checkout-success/page.tsx` accepts arbitrary `session_id` with no auth check.~~ **RESOLVED (Phase 113.2).** Auth guard added. Unauthenticated visitors redirected to `/sign-in`. | ~~HIGH~~ Resolved     |
| TD-WCAG-07 | A11y     | `AvatarMenu` dropdown lacks keyboard navigation: no `Escape` handler, no Arrow key navigation, no `role="menu"`/`role="menuitem"`. Phase 114.                                                                                   | MEDIUM                |
...
| TD-TASK-PASSTHROUGH | API      | ~~`createTaskSchema` in `task.actions.tsx` uses `.passthrough()` instead of `.strict()`.~~ **RESOLVED (Phase 113.1).** Elevated to CRITICAL as TD-SEC-10, now fixed. `.strict()` applied.                                                                                                                                      | ~~LOW~~ Resolved            |
...
| ID           | Area   | Description                                                                                                                                                                                                                                 | Severity              |
| ------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| TD-TEST-05   | Test   | ~~4 E2E specs fail (chat-app-shell, plans-public, public-pages, user-profile) due to hardcoded content assertions (prices, heading text, FAQ copy).~~ **RESOLVED (Phase 95.1–95.4)** — All E2E specs use structural assertions. 0 failures. | ~~CRITICAL~~ Resolved |
| TD-TEST-04   | Test   | ~~`conversation-stop.test.ts` is near-duplicate of `openai-route.test.ts` — same 13-mock setup duplicated.~~ **RESOLVED (Phase 96.1)** — File deleted, scenarios merged into openai-route.test.ts.                                          | ~~HIGH~~ Resolved     |
| TD-CONFIG-05 | Config | ~~ESLint has only 1 custom rule. Missing: `no-console`, `no-restricted-globals`.~~ **RESOLVED (Phase 94.3)** — 3 custom rules: `no-console` (error), `no-restricted-globals` (error), `set-state-in-effect` (warn).                         | ~~HIGH~~ Resolved     |
