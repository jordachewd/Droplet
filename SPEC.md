...
> Last updated: 2026-04-01 (PM audit #82). Milestones 0–25 COMPLETE. TDD rebuild COMPLETE (Phases 120.1–120.7). WCAG 2.2 AA COMPLETE. **DEPLOYED TO PRODUCTION — 2 CRITICAL BUGS ACTIVE (payment RE-OPENED, streaming).** BUG-PAYMENT RE-OPENED (PM audit #81 — webhook 200 OK but no Transaction/plan update; root cause likely Stripe Dashboard event selection, not code). BUG-STREAM active — streaming fails on media gen (Vercel 60s timeout, Phase 160.2 pending). BUG-AUDIO RESOLVED (Phase 168). BUG-HYDRATION RESOLVED (PM audit #81 verified — code already fixed). BUG-SCRIPT RESOLVED (PM audit #81 verified — code already fixed). TEST REGRESSION RESOLVED (PM audit #81 verified — 597 tests pass). Phase 167 PARTIAL (33 catch blocks remaining). Admin configurability PARTIAL (~25 hardcoded strings — Phase 162). 597 unit tests (101 suites). E2E: 49 tests (8 spec files). Coverage: 85/80/85/85. **All 7 Validation Gates GREEN.** Build passing. Node.js 24.12.0.
...
| ------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| TD-PAYMENT-02 | Billing | **RE-OPENED (PM audit #79).** Owner reports (2026-03-31): Stripe payment succeeds, webhook returns 200 OK, but NO Transaction created and NO User plan updated. Code logic quadruple-audited correct (PM audit #82). Webhook has ZERO event type logging — cannot determine which events are being received. Hypothesis: `checkout.session.completed` not reaching webhook, or non-checkout events returning 200 "Unhandled event" masking failure. Phase 169 adds diagnostic logging. | 169   |
...
| ID           | Area     | Description                                                                                                                                                                                                                                                        | Phase |
| ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| TD-HYDRA-01  | Frontend | ~~**CRITICAL.** Theme toggle hydration mismatch.~~ **RESOLVED (PM audit #81 verified, 2026-03-31).** Code already uses `useState<ThemeMode>("system")` with `useEffect` for localStorage read. No `getInitialMode` function exists. TODO was stale.                | 170   |
| TD-SCRIPT-01 | Frontend | ~~**CRITICAL.** Script tag error during rendering.~~ **RESOLVED (PM audit #81 verified, 2026-03-31).** `layout.tsx` already uses `<Script src="/scripts/theme-init.js">` (external file). `public/scripts/theme-init.js` exists with correct IIFE. TODO was stale. | 171   |
