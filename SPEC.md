...
> Last verified: PM audit #143, 2026-05-29. Milestones 0–25 COMPLETE. TDD rebuild COMPLETE (Phases 120.1–120.7). WCAG 2.2 AA COMPLETE. **V1.0 MVP RELEASED.** Phases 143–260 COMPLETE (all sub-phases). **Phase 253 COMPLETE — `/design` route deleted (commit 816dc81).** Onboarding wizard COMPLETE (Phases 240-251). **Phase 259 COMPLETE — all 14 code fixes verified in source, gates restored.** **Phase 260 COMPLETE — Mongo SRV DNS resilience + authenticated failure handling.** **⚠️ CURRENT GATE STATUS (audit #143, May 29 2026):** TypeScript GREEN (0 errors), Tests GREEN (113 suites / 745 tests / 0 failures), Lint GREEN (0 errors / 5 known `react-hooks/set-state-in-effect` warnings), Knip GREEN (unverified this session — assumed stable). Build unverified this session. **Active backlog: Phase 258 (owner decision required — UsageEvent TTL), TD-RELATIVE-IMPORTS-01 (LOW).** **Production deployment requires all 6 gates verified GREEN + Phase 258 decision.** TD-DB-DNS-01 RESOLVED. TD-USAGEEVENT-01 ACTIVE (owner decision pending). 9 Mongoose models confirmed. CSS architecture: 37 files, all imported in `index.css`.
>
> **Environment Variables (Phase 260 additions):**
>
> - `MONGODB_URL_FALLBACK` — optional non-SRV MongoDB connection string used as final fallback when SRV DNS resolution fails. Recommended for local dev on Windows with loopback-only DNS.
> - `DNS_FALLBACK_SERVERS` — optional comma-separated list of DNS server IPs to inject when loopback-only DNS is detected (default: `1.1.1.1,8.8.8.8`). Dead code path in Vercel production.
>
...
> - ✅ Phase 165.1 DONE — Plan-status route hardening (`maxDuration=60`, per-user rate limiting).
...
- Rate limiting: per-user sliding window (MongoDB-backed and persistent, via `RateLimitEntry` with a TTL index). Thresholds are configured per route and not published here.
...
> **TD-AUDIO-01 — RESOLVED.** Audio player `ERR_INVALID_STATE` error. Triple-audit root cause: (A) SSE controller race — `controllerClosed` boolean flag added, checked by `emitHeartbeat()` before enqueuing, (B) download route HTTP Range support implemented — `parseByteRangeHeader()`, `Accept-Ranges: bytes`, `206 Partial Content`, (C) audio player lifecycle hardened — `previousAudioUrlRef` reset in cleanup, `src=""` disposal, error event listener. All three paths implemented.
...
### Active — MEDIUM Priority (PM audit #139)
...
### Resolved — MEDIUM Priority (PM audit #138–139)

| ID                    | Area | Description                                                                                                                                                                                                                                                 | Phase | Resolution                                                                                                                                       |
| --------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| TD-WEBHOOK-LOG-01     | Ops  | `handleInvoicePaymentFailed()` called `logStripeWebhookError()` on the success path — normal billing outcome incorrectly classified as error. `not_matched` replay also logged as error.                                                                    | 254   | ✅ RESOLVED (PM audit #138, Phase 254). Success path and `not_matched` now use `logStripeWebhookInfo` with explicit idempotency message.         |
| TD-ENV-CONSISTENCY-01 | Code | `requireEnv()` inconsistently applied in 3 files: `api/webhooks/stripe/route.tsx`, `api/webhooks/clerk/route.tsx`, `(public)/checkout-success/page.tsx` used bare `process.env.*` with manual null guards. Risk: silent misconfiguration failure on deploy. | 255   | ✅ RESOLVED (PM audit #138, Phase 255). All 3 files use `requireEnv()`. Manual null guards removed. Fail-fast on misconfiguration.               |
| TD-SERVER-ACTION-01   | Arch | `incrementPromptCountIfBelowLimit` in `task.actions.ts` was exported as `"use server"` action but only called from the OpenAI API route. Architectural boundary misuse; clients could invoke it directly via Next.js server action mechanism.               | 256   | ✅ RESOLVED (PM audit #138, Phase 256). Moved to `task-queries.ts` (has `import "server-only"`). Not a server action. Auth and ownership intact. |
