...
> **Remaining Issues:**
>
> - **TD-PAYMENT-03** — 🔴 CRITICAL. Stripe webhook not delivering events post-payment. Plan stays Lite, no Transaction created, billing history empty. Webhook handler code is correct — root cause is Stripe Dashboard webhook configuration (endpoint URL, signing secret, enabled events, or mode mismatch). Owner must verify Stripe Dashboard. Stripe CLI was forwarding to wrong URL (`/api/stripe/webhook` instead of `/api/webhooks/stripe`). See PM audit #129, #130.
> - **TD-PROJECTION-01** — 🔴 CRITICAL. `USER_SYNC_PROJECTION` in `ensure-user-synced.ts` missing 4 top-level User fields: `stripeSubscriptionId`, `subscriptionStatus`, `stripeCustomerId`, `suspended`. Profile billing management always shows null for subscription data. Code fix required. See PM audit #129, #130.
> - **TD-BILLING-01** — 🔴 CRITICAL (NEW, PM audit #130). `getAllTransactions()` `.select()` projection excludes `stripeId`. `ProfileBilling` compares `txn.stripeId === stripeId` for Active/Inactive status — but `txn.stripeId` is always `undefined`, causing ALL billing history items to display as "Inactive". Code fix: add `stripeId` to `.select()` in `transaction.action.tsx`.
> - **TD-SYNC-01** — 🔴 CRITICAL (NEW, PM audit #130). MongoDB→Clerk sync is incomplete. `updateUser` server action only syncs avatar (`imageUrl`) to Clerk. `firstName` and `lastName` are NOT synced. When a user edits their name in the app, MongoDB is updated but Clerk retains stale values. The next `user.updated` webhook from Clerk will overwrite MongoDB with the old Clerk values — silently reverting the user's edits. Code fix: expand Clerk sync in `user.actions.tsx` to include `firstName` and `lastName`. Email sync requires Clerk verification flow and is deferred.
> - **TD-CHECKOUT-UX-01** — 🟡 HIGH. Checkout success page shows "Payment successful" from Stripe API verification before webhook processes. Plan status poller times out after 30s with misleading "plan will be updated shortly" message. No follow-up if plan never updates. Needs clearer timeout messaging with support contact. See PM audit #129, #130.
