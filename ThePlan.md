...
| Test infrastructure      | **TDD REBUILD IN PROGRESS (Phase 120.3 pending). Phase 129 COMPLETE (PageHead heading-level prop, FAQ h2 fix, E2E PersonaSpotlight locator fix, TDD tests). 511->514 tests.** 84 unit test suites, 514 tests (all pass). E2E: 14 spec files, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN. TDD infrastructure built (Phase 120.1). Phase 120.2 COMPLETE — all ~40 utility test files rebuilt. Phase 127 COMPLETE (AppSetting enum fix). Phase 128.1 COMPLETE (shared Button component + TDD tests). Phases 121–128.1 COMPLETE. 203 `as never` casts remain in 17 non-rebuilt test files (actions/routes/components/constants). Coverage: stmts ~85, branches ~72, funcs ~89, lines ~85. | ✅ All 7 gates GREEN. 514 tests (84 suites). E2E: 14 specs, 108 passed, 0 failed, 25 skipped. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
24. Testing infrastructure rebuild delivered through Phase 120.1 + 117 + 113.2 + 112.2 + 109 + 115 + 116. **TDD REBUILD IN PROGRESS.** Previous 532-test suite deleted. Current: **84 suites, 514 tests — all pass. E2E: 14 specs, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN.** Phase 120.2 COMPLETE — all ~40 utility test files rebuilt (480→511). Phase 127 COMPLETE (AppSetting enum fix). Phase 128.1 COMPLETE (shared Button component + TDD tests). Phases 121–128.1 COMPLETE. **Owner directive: FULL TDD REBUILD from scratch.**
25. Owner directives (March 2026 update — ESCALATED): **TDD REBUILD MANDATED.** Owner directive: remove ALL existing unit and E2E tests and rebuild entire testing process from scratch using strict TDD methodology. Previous incremental rebuild (Phases 88–103.4) is complete and archived. Previous 532-test suite has been deleted. Current 514 tests (84 suites) and 14 E2E spec files represent the partially-rebuilt baseline. Phase 120.2 COMPLETE — all utility tests rebuilt. Next: Phase 120.3 (action tests). Additional mandates: NO hardcoded data (everything admin-configurable), WCAG 2.2 AA full compliance, components as data consumers, code reuse maximized, server-side utilities, user removal cascade verified, knip stays clean, reduce unnecessary renders/leaks.
26. Owner directives (March 2026 — FINAL, consolidated): (a) **TDD rebuild — CRITICAL: IN PROGRESS. Old tests deleted. 514 tests (84 suites) rebuilt so far. Phase 120.2 COMPLETE — all ~40 utility files rebuilt. Next: Phase 120.3 (action tests).** (b) Full admin configurability NON-NEGOTIABLE — NO HARDCODED data, everything admin-configurable. Remaining: FAQ (74.2), landing copy (104), stop msgs (107). (c) WCAG 2.2 AA compliance — major fixes done (Phases 72, 97.1, 99.5, 101, 103), full audit pass remaining. PageHead heading-level fix DELIVERED (Phase 129 COMPLETE). (d) `npm run knip` must stay clean — CLEAN (0 findings, Phase 122 resolved orphan). (e) `/app/profile` plan limitations display — DELIVERED (Phase 66.1). (f) Code reuse maximized — TD-REUSE-01/02/03/05 ALL RESOLVED. Remaining: TD-REUSE-04 (Phase 106). (g) Client components must be pure data consumers — COMPLETE (Phase 73.1 + 73.3). (h) Reduce resource leaks — COMPLETE (Phase 102, TD-LEAK-01 RESOLVED). (i) User removal cascades — COMPLETE. (j) Components consume data via props — COMPLETE. (k) Reduce unnecessary renders — COMPLETE. (l) Admin configurability — core done, FAQ/landing/stop-reasons remaining. (m) Shared Button component — CREATED (Phase 128.1 COMPLETE). Migration pending (Phase 128.2).
27. PM audit #61 architect findings: ~~AppSetting Mongoose `category` enum missing "trial"~~ **RESOLVED (Phase 127).** `getAllTransactions()` unbounded query (no `.select()`/`.limit()` — Phase 125.2 HIGH). `handleError` propagates raw `error.message` to callers (Phase 126.1 HIGH). 203 `as never` casts remain in 17 non-rebuilt test files. ~~No shared `<Button>` component exists~~ **RESOLVED (Phase 128.1).** ~~`PageHead` always renders `<h1>`~~ **RESOLVED (Phase 129 COMPLETE).** Heading-level prop added, FAQ renders h2. All `findOneAndUpdate` calls verified `strict: true` (13 calls checked).
...
| F    | Validation Gate   | GREEN  | All 7 gates GREEN. Lint (0 errors, 6 warnings), knip (0 findings), TSC clean, unit (511/84 suites), E2E (14 specs, 108 passed, 25 skipped), build passes. |
...
> **All 7 gates GREEN.** 84 unit test suites, 514 tests (all pass). E2E: 14 specs, 108 passed, 0 failed, 25 skipped. Phase 129 COMPLETE.
...
> **Phase 120.2 COMPLETE** — All ~40 utility test files rebuilt from scratch (Batch A+B+C). 480→514 tests.
...
> **Phase 124 COMPLETE** — Merge conflict leftovers resolved (48 files).
> **Phase 125 COMPLETE** — Merge conflicts round 2 resolved (52 files from failed `git merge devel`).
> **Phase 126 COMPLETE** — Zod skill file conflict markers restored from devel.
> **Phase 127 COMPLETE** — AppSetting Mongoose `category` enum mismatch fixed. "trial" added.
> **Phase 128.1 COMPLETE** — Shared `<Button>` component created with TDD tests. First consumer migrated.

**Priority order (PM audit #62 — revised):**

1. ~~**Phase 129 HIGH** — PageHead heading-level prop + E2E locator fix~~ **COMPLETE (PM audit #62)**
2. **Phase 120.3 CRITICAL** — TDD server action test rebuild (densest `as never` concentration: 104 casts in 6 files)
3. **Phase 126.1 HIGH** — handleError error sanitization (must complete before 120.4 route tests — fragile pattern)
4. **Phase 125.2 HIGH** — Harden `getAllTransactions()` query (quick fix, high discipline value)
5. **Phase 128.2 HIGH** — Migrate 11 raw buttons to shared `<Button>` component (8 files)
6. **Phase 106 HIGH** — Extract shared `ChatApiResponse` types (before 120.4 to prevent duplication cement)
7. **Phase 120.4 CRITICAL** — TDD API route test rebuild (split openai-route)
8. **Phase 120.5 HIGH** — TDD component test rebuild (50 of 68 components untested)
9. **Phase 120.6 HIGH** — TDD E2E test rebuild
10. **Phase 120.7 HIGH** — Raise coverage thresholds post-rebuild
11. **Phase 107 HIGH** — Stop reason messages admin-configurable (9 hardcoded strings)
12. **Phase 108 MEDIUM** — Tablist arrow-key navigation (WCAG)
13. **Phase 114 MEDIUM** — AvatarMenu keyboard navigation (WCAG)
14. **Phase 126.2 LOW** — Lint warnings (6 setState-in-effect)
15. **Phase 125.1 MEDIUM** — Schema explicit strict:true
16. **Phase 74.2 MEDIUM** — FAQ content admin-configurable
17. **Phase 104 MEDIUM** — Landing/hero/about content admin-configurable
...
| ---- | ------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OI1  | TDD rebuild ALL tests from scratch                | CRITICAL | IN PROGRESS. Old tests deleted, rebuild underway. Current: 514 tests (84 suites). Phase 120.1 COMPLETE (TDD infra). Phase 120.2 COMPLETE — all ~40 utility files rebuilt. Phases 121–128.1 COMPLETE. 203 `as never` casts remain in 17 non-rebuilt test files (target: zero). Next: Phase 120.3 (action tests). |
...
> **Status: IN PROGRESS** — TDD rebuild underway. Previous 532-test suite deleted per owner directive. Current: 84 suites, 514 tests. Phase 120.2 COMPLETE — all ~40 utility test files rebuilt. Phase 127 COMPLETE (AppSetting enum fix). Phase 128.1 COMPLETE (shared Button component + TDD tests). Phase 129 COMPLETE (PageHead heading-level prop + FAQ h2 + E2E locator fix + TDD tests). Phases 121–129 COMPLETE. PM audit #62: no new CRITICAL bugs. Next: Phase 120.3 (CRITICAL TDD action test rebuild).
...
- ✅ TDD utility test rebuild from scratch — Phase 120.2 COMPLETE (all ~40 files).
...
- ✅ PageHead heading-level fix — Phase 129 COMPLETE.
- ⬜ Button migration — Phase 128.2 PENDING.
