...
| Billing mode             | Stripe recurring subscriptions with webhook processing and server-side price re-verification                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 🟡 CODE-COMPLETE — Phase 161 idempotency repair implemented, deployment unblocked (Phase 160.1 COMPLETE), awaiting deployment                | `SPEC.md`, Stripe webhook code, `checkoutPlan()`                                                |
...
41. ~~**CRITICAL — Payment/transaction still not registering (Phase 161).**~~ **RE-OPENED (PM audit #78, 2026-03-30).** Owner confirms: payment succeeds but no Transaction created, no User plan update after deployment. Code verified correct by triple-audit. Root cause: Stripe webhook endpoint misconfiguration in production (URL, signing secret, or event types). **Zero code changes needed. This is a pure ops fix.** Check Stripe Dashboard → Developers → Webhooks → Recent deliveries for evidence.
