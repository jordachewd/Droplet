...
The execution order is: **CRITICAL Vercel deployment blocker (Phase 160.1 maxDuration fix) → admin configurability promo text (Phase 162) → global error boundary (Phase 163) → missing maxDuration on other routes (Phase 166) → empty catch blocks (Phase 167) → env hardening (Phase 143) → performance (Phase 144 config cache) → remaining backlog.**
...
| Billing mode             | Stripe recurring subscriptions with webhook processing and server-side price re-verification                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | � CODE-COMPLETE — Phase 161 idempotency repair implemented, awaiting deployment                                                              | `SPEC.md`, Stripe webhook code, `checkoutPlan()`                                                |
| Streaming                | Streaming chat has SSE implementation with heartbeat (Phase 149) and catch/finally hardening (Phase 158). **120s auto-timeout safety net** (Phase 133). **Phase 160 CODE-COMPLETE:** `export const maxDuration = 300`, general heartbeat (30s), `didSendFinal` guard, stderr logging. **⚠️ BLOCKED: Vercel Hobby rejects maxDuration=300 (max 60). Phase 160.1 must reduce to 60.**                                                                                                                                                                                                                             | 🔴 CRITICAL — Vercel deployment blocked (Phase 160.1)                                                                                        | `src/app/api/openai/route.tsx`, `src/components/chat/chat-wrapper.tsx`                          |
...
| Stream error on media gen | ~~**CRITICAL production bug.** Media generation via tool calls triggers `Error: The response stream ended unexpectedly.`~~ Phase 149 (SSE heartbeat) + Phase 158 (catch/finally hardening) + **Phase 160 (maxDuration + heartbeat + didSendFinal + logging)** all applied. **CODE-COMPLETE.** | **🟡 CODE-COMPLETE — blocked by Vercel Hobby maxDuration (Phase 160.1).** | `src/app/api/openai/route.tsx`, `src/components/chat/chat-wrapper.tsx` |
...
| Payment checkout fix | ~~**CRITICAL production bug (PM audit #69).** `redirect()` inside try/catch in `checkoutPlan()`.~~ Phase 152 fixed redirect. Phase 157 fixed webhook schema (.strict()→.strip()). **Phase 161 CODE-COMPLETE:** Idempotency repair checks Transaction + User plan.stripeId. Repair path for stale plan state. Top-level try/catch. Full error logging. **Awaiting deployment (blocked by Phase 160.1) + Stripe Dashboard ops check.** | **🟡 CODE-COMPLETE — awaiting deployment + ops verification.** | `src/app/api/webhooks/stripe/route.tsx`, `src/lib/actions/transaction.action.tsx` |
...
40. ~~**CRITICAL — Stream error persists in production (Phase 160).**~~ **CODE-COMPLETE (Phase 160, PM audit #75).** `maxDuration=300`, general+media heartbeats, `didSendFinal` guard, stderr logging all implemented. **⚠️ BUT: Vercel Hobby rejects `maxDuration=300` (max 60). Phase 160.1 must reduce to 60. Video gen (up to 180s) cannot complete on Hobby — owner decision: accept 60s or upgrade to Vercel Pro ($20/mo).**
41. ~~**CRITICAL — Payment/transaction still not registering (Phase 161).**~~ **CODE-COMPLETE (Phase 161, PM audit #75).** Idempotency repair: checks Transaction + User `plan.stripeId`. Repair path via `applyCheckoutPlanUpdate()`. Top-level try/catch with session ID context. All error details logged to stderr. **Awaiting production deployment (blocked by Phase 160.1) + Stripe Dashboard ops verification.**
...
43. **🔴 CRITICAL — Vercel Hobby `maxDuration` deployment blocker (Phase 160.1, PM audit #75).** Vercel production build error: "Builder returned invalid maxDuration value for Serverless Function 'api/openai'. Serverless Functions must have a maxDuration between 1 and 60 for plan hobby." `maxDuration=300` set in Phase 160 requires Vercel Pro. Deployment is completely blocked. Fix: reduce to 60. Trade-off: video gen may time out on Hobby. Text chat + image + audio should fit within 60s.
...
| C    | Product Gate      | RED    | 6 personas plan-gated, stop reasons working, trial access live. Streaming + payment **code-complete** (Phases 160+161) but **blocked from deploying** by Vercel Hobby `maxDuration` limit (Phase 160.1).                                                             |
...
| F    | Validation Gate   | GREEN  | All 7 validation commands pass locally. Lint (0/0), knip (0), TSC clean, unit (101 suites, 592 tests), E2E (8 specs, 49 tests), build passes, coverage 85/80/85/85. Zero `as never` casts. **Vercel production build FAILS** due to `maxDuration=300` on Hobby plan. |

---

## 9. Current Execution Order

> Milestones 0–25 ALL COMPLETE. Phases 160 (stream fix), 161 (payment fix), 164 (client timeout) CODE-COMPLETE. Phase 141 (suspended user enforcement) COMPLETE. Phase 149 (SSE heartbeat streaming fix) COMPLETE. Phases 150–155 ALL COMPLETE. Phase 157 (Stripe webhook schema fix) COMPLETE. Phase 155.1 (scrollbar cleanup) COMPLETE. Phase 158 (SSE catch/finally hardening) COMPLETE. Phase 159 (button test fix) COMPLETE.
> **5 of 7 gates GREEN (Product RED, Admin YELLOW).** 101 unit test suites, 592 tests (all pass). E2E: 8 specs (49 tests). Zero `as never` casts.
...
> **🔴 CRITICAL DEPLOYMENT BLOCKER:** Vercel Hobby plan rejects `maxDuration=300`. Must reduce to 60 (Phase 160.1). All code fixes blocked from reaching production.

**Priority order (PM audit #75 — deployment-blocker-first):**

1. **Phase 160.1 🔴 CRITICAL** — Reduce `maxDuration` from 300 to 60 for Vercel Hobby compliance. Re-align client timeout from 310_000 to 70_000. Unblocks deployment of ALL code fixes (Phases 160, 161, 164). **Trade-off:** Video gen (up to 180s Sora) will time out on Hobby. Text + image + audio should fit within 60s. Owner can upgrade to Vercel Pro for 300s support.
2. **Phase 162 HIGH** — Extract hardcoded promo/upgrade text from `chat-sidebar-promo.tsx`, `plan-promo.tsx`, `persona-card.tsx` to admin-configurable settings via `effective-*` resolver pattern.
3. **Phase 163 HIGH** — Add `global-error.tsx` for root layout error recovery.
4. **Phase 166 HIGH** — Add `maxDuration` exports to 5 API routes missing them (upload, download, aws, webhooks/stripe, webhooks/clerk). All 30s.
5. **Phase 167 HIGH** — Fix 2 empty catch blocks violating AGENTS.md (chat-sidebar-shell.tsx, chat-sidebar.tsx).
...
- Phase 160 — `maxDuration=300`, heartbeat, `didSendFinal`, stderr logging. (Needs 160.1 to deploy.)
- Phase 161 — Webhook idempotency repair. (Needs 160.1 to deploy + Stripe Dashboard ops check.)
- Phase 164 — Client timeout 310s. (Needs 160.1 to re-align to 70s.)
