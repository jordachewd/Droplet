...
| Test infrastructure      | **TDD REBUILD COMPLETE (Phases 120.1–120.7 ALL DONE).** 97 unit test suites, 561 tests (all pass). E2E: 8 spec files. Build passing. All 7 gates GREEN. Zero `as never` casts (was 99, now 0). Coverage thresholds: 85/80/85/85. OpenAI route tests split into 5 focused modules ≤321 lines each. 24 component test files. TDD methodology enforced throughout. 3 new E2E specs added (Phase 134): billing checkout, admin user ops, error boundary. | ✅ TDD REBUILD COMPLETE. All 7 gates GREEN. 561 tests (97 suites). E2E: 8 specs. Zero `as never`. Coverage: 85/80/85/85. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
| Payment checkout fix | ~~**CRITICAL production bug (PM audit #69).** `redirect()` inside try/catch in `checkoutPlan()`.~~ Phase 152 fixed redirect. **NEW CRITICAL BUG (PM audit #71, Phase 157):** Stripe webhook `checkoutSessionMetadataSchema.strict()` rejects the `name` field sent by `checkoutPlan()`. `safeParse` fails → no Transaction created → no User plan updated. **P0 money-loss: customer charged but stays on Lite.** | ❌ Phase 152 fixed redirect only. Webhook schema mismatch blocks payment processing. Phase 157 CRITICAL. | `src/app/api/webhooks/stripe/route.tsx` L45–51, `src/lib/actions/transaction.action.tsx` L95 |
...
24. Testing infrastructure rebuild COMPLETE through Phases 120.1–120.7 + 117 + 113.2 + 112.2 + 109 + 115 + 116. **TDD REBUILD DONE.** Previous 532-test suite deleted. Current: **97 suites, 561 tests — all pass. E2E: 8 spec files. Build passing. All 7 gates GREEN.** Phase 120.2 COMPLETE — all ~40 utility test files rebuilt. Phase 120.3 COMPLETE — all action tests rebuilt. Phase 120.4 COMPLETE — route tests rebuilt, openai split into 5 modules. Phase 120.5 COMPLETE — 24 component test files. Phase 120.6 COMPLETE — E2E rebuilt (5 specs, structural assertions). Phase 120.7 COMPLETE — thresholds raised to 85/80/85/85. Phase 134 COMPLETE — 3 new E2E specs (billing checkout, admin user ops, error boundary). Zero `as never` casts remaining.
25. Owner directives (March 2026 — FINAL): (a) **TDD rebuild — COMPLETE.** 561 tests (97 suites). 8 E2E specs. Zero `as never`. Coverage 85/80/85/85. Phases 120.1–120.7 ALL COMPLETE. (b) **Full admin configurability — COMPLETE.** FAQ (74.2) ✅, landing copy (104) ✅, stop msgs (107) ✅. Zero remaining gaps. (c) **WCAG 2.2 AA compliance — FULLY COMPLETE.** Tabs (108) ✅, AvatarMenu (114) ✅. All other sub-directives COMPLETE: code reuse (TD-REUSE-01–05 resolved), data consumers (73.1+73.3), resource leaks (102), user cascades, server-side utilities, knip clean, shared Button (128.1+128.2), shared types (106).
...
34. ~~**CRITICAL production bug — payment checkout broken (PM audit #69, Phase 152).**~~ **PARTIALLY RESOLVED (Phase 152).** Redirect fixed. **But webhook schema mismatch (TD-WEBHOOK-01) prevents payment processing.** Phase 157 CRITICAL tracks the fix.
...
38. **CRITICAL production bug — Stripe webhook silently drops payment processing (PM audit #71, Phase 157).** `checkoutPlan()` sends `name: fullName` in metadata. Webhook `checkoutSessionMetadataSchema.strict()` rejects extra keys. `safeParse` fails → no Transaction → no plan update. **Customer charged by Stripe but stays on Lite.** Root cause: `.strict()` vs `.strip()` on Zod schema + unused `name` field in metadata. Triple-confirmed by Architect, Engineer, and PM.
