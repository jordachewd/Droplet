...
| Node.js runtime          | Node.js 24.12.0 runtime confirmed. `@types/node@^25.5.0`. Phase 76 delivered: `server-only` guards on 6 utility files, `knip` audit, MongoDB SRV-failure resilience, self-heal dedupe/throttling, Next.js smooth-scroll warning fix. **Note (PM audit #37):** `.nvmrc` and `engines` field claimed in Phase 76 but not found in repo. Phase 81 created to properly pin Node 24.12.0.                                                               | ⚠️ Runtime confirmed, version-pin artifacts missing (Phase 81)                       | `package.json`, Phase 76 deliverables                                                           |
...
17. Node.js 24.12.0 runtime confirmed. `.nvmrc` and `engines` field missing — Phase 81 addresses this.
18. limeGreen brand color change from `#B8F60D` to `#D9F20C` pending — Phase 82.
...
20. `knip` reports 2 findings: unused `@typescript-eslint/parser` devDep, unused `chatTools` export. Phase 83.
21. 2 server utilities missing `server-only` guard: `ai-model-policy.ts`, `check-usage-limit.ts`. Phase 84.
...
16. ✅ Node.js stabilization (Phase 76): repo-wide error scan, `server-only` guards, `knip` cleanup, MongoDB SRV resilience, self-heal dedupe, Next.js scroll warning fix, all six validation gates passed. Runtime upgraded to 24.12.0 (PM audit #37). `.nvmrc` and `engines` field still missing — Phase 81.
...
| --- | ------------------------------------------------------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Node.js 24.12.0 compatibility                                | CRITICAL | ⚠️ PARTIAL. Runtime is 24.12.0. `.nvmrc` and `engines` field missing. Phase 81 addresses.                                                                            |
...
| 9   | npm run knip — unused code audit                             | HIGH     | ⚠️ 2 findings remaining: unused `@typescript-eslint/parser` devDep, unused `chatTools` export. Phase 83.                                                             |
...
| #   | Directive                                                                    | Priority | Status                                                                                                                                                            |
| --- | ---------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 12  | Brand color palette: add Lime Green (#D9F20C) as NEW accent                  | HIGH     | ✅ IMPLEMENTED (Phase 68.1). limeGreen palette 100–1000 added to theme tokens. Applied across button styles. Base hex updated from #B8F60D to #D9F20C (Phase 82). |
...
> **Status: MOSTLY COMPLETE** — Phase 76 delivered core hardening. Runtime upgraded to Node 24.12.0 (PM audit #37). Remaining: Phase 81 (version-pin artifacts), Phase 83 (2 knip findings), Phase 84 (2 server-only guards).
