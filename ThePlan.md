...
| Test infrastructure      | **TDD REBUILD IN PROGRESS (Phase 120). Phase 120.3 COMPLETE (all 4 action test files rebuilt, zero `as never`). Phase 126.1 COMPLETE (handleError sanitization). Phase 125.2 COMPLETE (getAllTransactions hardening). Phase 130 COMPLETE (status-routes test fix).** 82 unit test suites, 538 tests (all pass). E2E: 14 spec files, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN. TDD infrastructure built (Phase 120.1). Phase 120.2 COMPLETE — all ~40 utility test files rebuilt. Phase 127 COMPLETE (AppSetting enum fix). Phase 128.1 COMPLETE (shared Button component + TDD tests). Phases 121–130 COMPLETE. 99 `as never` casts remain in 11 non-rebuilt test files (6 routes, 4 components, 1 constants). Coverage: stmts ~85, branches ~72, funcs ~89, lines ~85. | ✅ All 7 gates GREEN. 538 tests (82 suites). E2E: 14 specs, 108 passed, 0 failed, 25 skipped. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
24. Testing infrastructure rebuild delivered through Phase 120.1 + 117 + 113.2 + 112.2 + 109 + 115 + 116. **TDD REBUILD IN PROGRESS.** Previous 532-test suite deleted. Current: **82 suites, 538 tests — all pass. E2E: 14 specs, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN.** Phase 120.2 COMPLETE — all ~40 utility test files rebuilt (480→511). Phase 120.3 COMPLETE — all action tests rebuilt. Phase 127 COMPLETE (AppSetting enum fix). Phase 128.1 COMPLETE (shared Button component + TDD tests). Phase 125.2 COMPLETE (getAllTransactions hardened). Phase 126.1 COMPLETE (handleError sanitized). Phase 130 COMPLETE (status-routes test fix). Phases 121–130 COMPLETE. **Owner directive: FULL TDD REBUILD from scratch.**
25. Owner directives (March 2026 update — ESCALATED): **TDD REBUILD MANDATED.** Owner directive: remove ALL existing unit and E2E tests and rebuild entire testing process from scratch using strict TDD methodology. Previous incremental rebuild (Phases 88–103.4) is complete and archived. Previous 532-test suite has been deleted. Current 538 tests (82 suites) and 14 E2E spec files represent the partially-rebuilt baseline. Phase 120.2 COMPLETE — all utility tests rebuilt. Phase 120.3 COMPLETE (action tests rebuilt). Phase 125.2 COMPLETE (getAllTransactions hardened). Phase 126.1 COMPLETE (handleError sanitized). Phase 130 COMPLETE (status-routes fix). Next: Phase 128.2 (Button migration). Additional mandates: NO hardcoded data (everything admin-configurable), WCAG 2.2 AA full compliance, components as data consumers, code reuse maximized, server-side utilities, user removal cascade verified, knip stays clean, reduce unnecessary renders/leaks.
26. Owner directives (March 2026 — FINAL, consolidated): (a) **TDD rebuild — CRITICAL: IN PROGRESS. Old tests deleted. 538 tests (82 suites) rebuilt so far. Phase 120.3 COMPLETE — all action tests rebuilt. 99 `as never` casts remain. Next: Phase 128.2 (Button migration) → 106 → 120.4.** (b) Full admin configurability NON-NEGOTIABLE — NO HARDCODED data, everything admin-configurable. Remaining: FAQ (74.2), landing copy (104), stop msgs (107). (c) WCAG 2.2 AA compliance — major fixes done (Phases 72, 97.1, 99.5, 101, 103), full audit pass remaining. PageHead heading-level fix DELIVERED (Phase 129 COMPLETE). (d) `npm run knip` must stay clean — CLEAN (0 findings, Phase 122 resolved orphan). (e) `/app/profile` plan limitations display — DELIVERED (Phase 66.1). (f) Code reuse maximized — TD-REUSE-01/02/03/05 ALL RESOLVED. Remaining: TD-REUSE-04 (Phase 106). (g) Client components must be pure data consumers — COMPLETE (Phase 73.1 + 73.3). (h) Reduce resource leaks — COMPLETE (Phase 102, TD-LEAK-01 RESOLVED). (i) User removal cascades — COMPLETE. (j) Components consume data via props — COMPLETE. (k) Reduce unnecessary renders — COMPLETE. (l) Admin configurability — core done, FAQ/landing/stop-reasons remaining. (m) Shared Button component — CREATED (Phase 128.1 COMPLETE). Migration pending (Phase 128.2).
...
| F    | Validation Gate   | GREEN  | All 7 gates GREEN. Lint (0 errors, 6 warnings), knip (0 findings), TSC clean, unit (538/82 suites), E2E (14 specs, 108 passed, 25 skipped), build passes. |
...
> **All 7 gates GREEN.** 82 unit test suites, 538 tests (all pass). E2E: 14 specs, 108 passed, 0 failed, 25 skipped. Phases 120.3, 125.2, 126.1, 129, 130 COMPLETE.
...
> **Phases 120.1–120.3 COMPLETE** — TDD infrastructure + all utility tests + all action tests rebuilt. Zero `as never` in rebuilt files.
...
> **Phase 128.1 COMPLETE** — Shared `<Button>` component created with TDD tests.
> **Phase 130 COMPLETE** — Status-routes test assertion fix ("HTTP" → "ERROR:").
> **99 `as never` casts remain** in 11 non-rebuilt test files (6 routes, 4 components, 1 constants).

**Priority order (PM audit #63):**

1. **Phase 128.2 HIGH** — Migrate 11 raw buttons to shared `<Button>` component (8 files)
2. **Phase 106 HIGH** — Extract shared `ChatApiResponse` types (before 120.4 to prevent duplication cement)
3. **Phase 120.4 CRITICAL** — TDD API route test rebuild (eliminates 88 of 99 `as never` casts)
4. **Phase 120.5 HIGH** — TDD component test rebuild (51 of 69 components untested)
5. **Phase 120.6 HIGH** — TDD E2E test rebuild
6. **Phase 120.7 HIGH** — Raise coverage thresholds post-rebuild
7. **Phase 107 HIGH** — Stop reason messages admin-configurable (9 hardcoded strings)
8. **Phase 108 MEDIUM** — Tablist arrow-key navigation (WCAG)
9. **Phase 114 MEDIUM** — AvatarMenu keyboard navigation (WCAG)
10. **Phase 131 LOW** — Add server-only guards to 4 utility files
11. **Phase 132 LOW** — Resolve stale TODO comments in ai-model-policy.ts
12. **Phase 126.2 LOW** — Lint warnings (6 setState-in-effect)
13. **Phase 125.1 MEDIUM** — Schema explicit strict:true
14. **Phase 74.2 MEDIUM** — FAQ content admin-configurable
15. **Phase 104 MEDIUM** — Landing/hero/about content admin-configurable
...
| ---- | ------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| OI1  | TDD rebuild ALL tests from scratch                | CRITICAL | IN PROGRESS. Old tests deleted, rebuild underway. Current: 538 tests (82 suites). Phase 120.1–120.3 COMPLETE (TDD infra + utilities + actions). Phase 125.2 COMPLETE (transactions). Phase 126.1 COMPLETE (handleError). Phase 130 COMPLETE (status-routes fix). 99 `as never` casts remain in 11 non-rebuilt test files (6 routes, 4 components, 1 constants). Next: Phase 128.2 (Button migration) → 106 (shared types) → 120.4 (route tests). |
...
> **Status: IN PROGRESS** — TDD rebuild underway. Previous 532-test suite deleted per owner directive. Current: 82 suites, 538 tests. Phases 120.1–120.3, 125.2, 126.1, 127–130 all COMPLETE. 99 `as never` casts remain in 11 non-rebuilt test files. PM audit #63: status-routes test fix (Phase 130). No new CRITICAL bugs. Next: Phase 128.2 (Button migration) → 106 (shared types) → 120.4 (route tests — eliminates 88 of 99 `as never` casts).
...
6. **CRITICAL** — Phase 120.3: TDD server action test rebuild — **COMPLETE** (514→538 tests, zero `as never`).
...
- ⬜ Button migration — Phase 128.2 PENDING (next).
- ⬜ Extract shared types — Phase 106 PENDING.
