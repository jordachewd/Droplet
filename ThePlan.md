...
| Test infrastructure      | **TDD REBUILD IN PROGRESS (Phase 120.2).** 68 unit test suites, 419 tests (all pass). E2E: 14 spec files, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN. TDD infrastructure built (Phase 120.1). Phase 120.2 Batch A COMPLETE (6 IO/business-critical utility files). 26 utility test files rebuilt out of ~40 target. Previous 532-test suite deleted per owner directive; rebuild from scratch underway. Phases 121–126 COMPLETE. Phase 126: 47 zod skill conflict markers resolved. | ✅ All 7 gates GREEN. 419 tests (68 suites). E2E: 14 specs, 108 passed, 0 failed, 25 skipped. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
24. Testing infrastructure rebuild delivered through Phase 120.1 + 117 + 113.2 + 112.2 + 109 + 115 + 116. **TDD REBUILD IN PROGRESS.** Previous 532-test suite deleted. Current: **68 suites, 419 tests — all pass. E2E: 14 specs, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN.** Phase 120.2 Batch A complete (6 IO files hardened, 409→419). 26 of ~40 utility test files rebuilt. Phases 121–126 COMPLETE. **Owner directive: FULL TDD REBUILD from scratch.**
25. Owner directives (March 2026 update — ESCALATED): **TDD REBUILD MANDATED.** Owner directive: remove ALL existing unit and E2E tests and rebuild entire testing process from scratch using strict TDD methodology. Previous incremental rebuild (Phases 88–103.4) is complete and archived. Previous 532-test suite has been deleted. Current 409 tests (68 suites) and 14 E2E spec files represent the partially-rebuilt baseline. Additional mandates: NO hardcoded data (everything admin-configurable), WCAG 2.2 AA full compliance, components as data consumers, code reuse maximized, server-side utilities, user removal cascade verified, knip stays clean, reduce unnecessary renders/leaks.
26. Owner directives (March 2026 — FINAL, consolidated): (a) **TDD rebuild — CRITICAL: IN PROGRESS. Old tests deleted. 419 tests (68 suites) rebuilt so far. Phase 120.2 Batch A COMPLETE. 26/~40 utility files rebuilt.** (b) Full admin configurability NON-NEGOTIABLE — NO HARDCODED data, everything admin-configurable. Remaining: FAQ (74.2), landing copy (104), stop msgs (107). (c) WCAG 2.2 AA compliance — major fixes done (Phases 72, 97.1, 99.5, 101, 103), full audit pass remaining. (d) `npm run knip` must stay clean — CLEAN (0 findings, Phase 122 resolved orphan). (e) `/app/profile` plan limitations display — DELIVERED (Phase 66.1). (f) Code reuse maximized — TD-REUSE-01/02/03/05 ALL RESOLVED. Remaining: TD-REUSE-04 (Phase 106). (g) Client components must be pure data consumers — COMPLETE (Phase 73.1 + 73.3). (h) Reduce resource leaks — COMPLETE (Phase 102, TD-LEAK-01 RESOLVED). (i) User removal cascades — COMPLETE. (j) Components consume data via props — COMPLETE. (k) Reduce unnecessary renders — COMPLETE. (l) Admin configurability — core done, FAQ/landing/stop-reasons remaining.
...
| F    | Validation Gate   | GREEN  | All 7 gates GREEN. Lint (0 errors, 6 warnings), knip (0 findings), TSC clean, unit (419/68 suites), E2E (14 specs, 108 passed, 25 skipped), build passes. |
...
> **All 7 gates GREEN.** 68 unit test suites, 409 tests (all pass). E2E: 14 specs, 108 passed, 0 failed, 25 skipped.
...
| ---- | ------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| OI1  | TDD rebuild ALL tests from scratch                | CRITICAL | IN PROGRESS. Old tests deleted, rebuild underway. Current: 419 tests (68 suites). Phase 120.1 COMPLETE (TDD infra). Phase 120.2 Batch A COMPLETE (6 IO files, 409→419). 26/~40 utility files rebuilt. Phases 121–126 COMPLETE. |
...
> **Status: IN PROGRESS** — TDD rebuild underway. Previous 532-test suite deleted per owner directive. Current: 68 suites, 419 tests. Phase 120.2 Batch A complete; Batch B+C in progress (26/~40 utility test files rebuilt). Phases 121–126 COMPLETE.
