...
> PM audit #104 (2026-04-08). Sidebar restructure COMPLETE (Phases 209–216). Phase 218 COMPLETE (CSS modular architecture). Phase 219 COMPLETE (orphan cleanup). **Phase 220 COMPLETE (useIsDesktop hook).** **Phase 221 COMPLETE (shared layout/form CSS).** Phase 222 ACTIVE (shared layout components). **Stripe recurring billing UNBLOCKED** (Phases 217-A–G).
...
- **Phase 217-A** — Schema + Stripe Product setup. Add `stripeCustomerId`, `stripeSubscriptionId`, `subscriptionStatus` fields to User model. Add `type`, `stripeInvoiceId` fields to Transaction model. Create 4 recurring Stripe Prices (Pro Monthly/Yearly, Premium Monthly/Yearly). Store Price IDs in AppSetting. Additive schema changes only — no behavior change. **Dependencies:** Stripe Dashboard product creation. **Effort:** ~30min. **Risk:** Low (additive).
- **Phase 217-B** — Checkout mode switch + Stripe Customer management. Switch `mode: "payment"` → `mode: "subscription"`. Create/retrieve Stripe Customer before checkout. Use persistent Price IDs instead of inline `price_data`. Add yearly checkout option. **Dependencies:** Phase 217-A. **Effort:** ~45min. **Risk:** Medium.
...
| OI57 | Reuse styles, functions, utilities, components across admin/chat        | ⏳ ACTIVE. Phases 219–222. 219+220+221 DONE. 222 remaining.                                                                                                     |
