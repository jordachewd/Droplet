...
4. **ToggleTheme hydration mismatch (NEW)** — `DropletTheme` reads localStorage during `useState` initialization, producing server/client state divergence. `ToggleTheme` renders different `aria-checked` and tooltip text. Same bug pattern as Phase 153. WCAG 2.2 AA regression.
5. **Script tag warning in root layout (NEW)** — `<Script strategy="beforeInteractive">` inside body JSX triggers React warning in Next.js 16.2.2.

The execution order is: **ToggleTheme hydration fix (Phase 169) → Script tag fix (Phase 170) → Proactive timeout safety net (Phase 160.2) → Remaining catch blocks (Phase 167.2) → Admin promo text (Phase 162) → Global error boundary (Phase 163) → Env hardening (Phase 143) → Performance (Phase 144 config cache) → Remaining backlog.**
...
| ToggleTheme hydration mismatch | **NEW CRITICAL bug (owner-reported, PM audit #79, 2026-04-01).** `DropletTheme` component reads localStorage during `useState(getInitialMode)` initialization. Server returns `mode="system"` → `resolvedMode="light"`. Client returns `mode="dark"` from localStorage → `resolvedMode="dark"`. `ToggleTheme` derives `darkActive` from context, producing `aria-checked="false"` / "Dark Mode" on server but `aria-checked={true}` / "Light Mode" on client. Same bug class as Phase 153 (AdminSettingsTabs hydration fix). Also regresses WCAG 2.2 AA (`aria-checked` must match component state). **Phase 169 tracks fix.** | **🔴 NEW — Phase 169.** | `src/components/layout/droplet-theme.tsx`, `src/components/shared/toggle-theme.tsx` |
| Script tag warning in root layout | **NEW HIGH bug (owner-reported, PM audit #79, 2026-04-01).** `<Script id="theme-init" strategy="beforeInteractive">` in `src/app/layout.tsx` renders inside body JSX. Next.js 16.2.2 renders the Script component inline during hydration, triggering React warning: "Scripts inside React components are never executed when rendering on the client." Fix: replace with raw `<script dangerouslySetInnerHTML>` in `<head>`. **Phase 170 tracks fix.** | **🟡 NEW — Phase 170.** | `src/app/layout.tsx` |
...
10. ~~Entire application must comply with **WCAG 2.2 AA** accessibility standards.~~ **PARTIAL (PM audit #79).** All sub-phases delivered including tablist arrow-key nav (108) and AvatarMenu keyboard nav (114). **REGRESSED:** ToggleTheme hydration mismatch causes `aria-checked` to not match component state on initial render (Phase 169). WCAG status will be restored to COMPLETE after Phase 169 resolves.
...
46. **NEW CRITICAL — ToggleTheme hydration mismatch (Phase 169, PM audit #79, 2026-04-01).** `DropletTheme.getInitialMode()` reads localStorage during `useState` init, producing different values server vs client. Same bug pattern as Phase 153 (AdminSettingsTabs). Fix: always initialize as `"system"`, sync from localStorage in `useEffect` after mount. The inline `<script>` in layout.tsx correctly sets `data-droplet-theme` for visual FOUC prevention — the issue is purely React state hydration. Also regresses WCAG 2.2 AA (`aria-checked` mismatch).
47. **NEW HIGH — Script tag warning in root layout (Phase 170, PM audit #79, 2026-04-01).** Next.js 16.2.2 `<Script strategy="beforeInteractive">` inside body JSX triggers React's script tag warning during hydration. Fix: replace with raw `<script dangerouslySetInnerHTML>` inside `<head>` element.
...
**4 of 6 gates GREEN. Product Gate RED, Admin Gate YELLOW.** DEPLOYED but 2 new bugs + 1 CRITICAL production bug active (PM audit #79, 2026-04-01).
...
| C    | Product Gate      | RED    | 1 CRITICAL production bug: stream error on media gen (Vercel 60s timeout). 2 NEW bugs: ToggleTheme hydration mismatch (Phase 169), Script tag warning (Phase 170). Audio player RESOLVED (Phase 168). Payment RESOLVED (PM audit #78.1). |
...
| F    | Validation Gate   | GREEN  | All 7 validation commands pass locally. Lint (0/0), knip (0), TSC clean, unit (101 suites, 592 tests), E2E (8 specs, 49 tests), build passes, coverage 85/80/85/85. Zero `as never` casts. `maxDuration=60` (Vercel Hobby compliant).    |

---

## 9. Current Execution Order

> Milestones 0–25 ALL COMPLETE. **DEPLOYED TO PRODUCTION.** 1 CRITICAL production bug active (streaming timeout). 2 NEW bugs (hydration mismatch + script tag warning). Phase 168 CODE-COMPLETE (audio player fix — archived to DONE.md). BUG-PAYMENT RESOLVED (PM audit #78.1 — owner verified). Phase 167 partially completed (targeted catches in 9 files). 34 parameterless catches remain repo-wide. Phase 169 NEW (ToggleTheme hydration). Phase 170 NEW (Script tag warning).
...
> 2. ~~**BUG-AUDIO**~~: **RESOLVED (Phase 168 CODE-COMPLETE, archived to DONE.md PM audit #79).**
> 3. **BUG-HYDRATION (NEW)**: ToggleTheme hydration mismatch — server/client state divergence. `aria-checked` wrong on initial render. **Phase 169.**
> 4. **BUG-SCRIPT (NEW)**: Script tag warning in root layout — React warning on hydration. **Phase 170.**
> 5. **BUG-STREAM**: Stream ends unexpectedly on media gen. Vercel 60s timeout. **Phase 160.2 — proactive timeout safety net.**

**Priority order (PM audit #79, updated 2026-04-01 — bugs first, then hardening):**

1. **🔴 Phase 169 CRITICAL** — ToggleTheme hydration mismatch fix: (a) change `getInitialMode()` to always return `"system"` (remove localStorage read from initializer); (b) change `systemMode` initializer to always return `"light"` (remove `window` access); (c) add `useEffect` to sync from localStorage after mount (same pattern as Phase 153); (d) verify `ToggleTheme` `aria-checked` and tooltip text match post-hydration.
2. **🔴 Phase 170 HIGH** — Script tag warning fix: (a) replace `<Script id="theme-init" strategy="beforeInteractive">` with raw `<script dangerouslySetInnerHTML={{__html: ...}}>` inside `<head>` element; (b) remove `next/script` import if no longer used; (c) verify no React warnings during hydration.
3. **🔴 Phase 160.2 CRITICAL** — Proactive timeout safety net: 55s timer sends graceful error before Vercel kill.
4. **Phase 167.2 HIGH** — Remaining 34 empty catch blocks across `src/`.
...
7. **Phase 165 MEDIUM** — Checkout success page DB polling (safety net — payment now working, deprioritized).
...
**Completed and deployed (Production) — recently archived:**

- Phase 168 — Audio player controller guard + download Range support + audio lifecycle hardening. **CODE-COMPLETE (verified PM audit #79).** Archived to DONE.md.

**Completed and deployed (Production):**

- Phase 160/160.1 — `maxDuration=60`, heartbeat, `didSendFinal`, stderr logging. **Deployed but insufficient (see Phase 160.2).**
...
| OI4  | WCAG 2.2 AA compliance                            | HIGH     | **PARTIAL (PM audit #79).** All sub-phases delivered. Tabs (108) ✅, AvatarMenu (114) ✅. **REGRESSED:** ToggleTheme hydration mismatch (`aria-checked` wrong on initial render). Will be restored to COMPLETE after Phase 169.                           |
