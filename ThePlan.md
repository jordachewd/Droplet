...
| Test infrastructure      | 65 unit test files organized by domain. Phase-suffixed duplicates consolidated. Failure path tests for media generation. Security injection tests. E2E specs consolidated. Error state E2E added. Server-only credential guards delivered (Phase 92). Extended server-only guards on 17 files (Phase 86). Admin table selection clearing tests (Phase 90.6). Admin bulk E2E, cleanup, behavioral tests (Phase 91.2–91.4). Phase 94.1–94.5 COMPLETE. Phase 95.1–95.4 COMPLETE. Phase 95-R COMPLETE (Chromium failures resolved — Playwright workers=1, resolvePlaywrightWorkers()). Phase 96.1 COMPLETE (conversation-stop merged). Phase 96.2 COMPLETE (pure utility un-mocking). Phase 96.3 COMPLETE (webhook idempotency/edge-case tests). Phase 99.1 COMPLETE (aria-live). Phase 99.3 COMPLETE (stable message keys via createMessageId). Phase 100.1–100.3 COMPLETE (isObjectRecord/VALID_PERSONA_ID_SET/legalReviewDisclaimer deduplication). **72 unit test suites, 433 tests (all pass). E2E: 108 passed, 0 failed, 25 skipped. Coverage: 78.18/65.94/83.01/78.51. Build passing.** Phase 96.5 COMPLETE (upload file size validation tests). Phase 96.6 COMPLETE (Zustand store tests). Phase 96.7 COMPLETE (component tests). Phase 96.8 COMPLETE (User model test expansion). Phase 99.2 COMPLETE (ConfirmationModal native dialog). Phase 99.4 COMPLETE (knip cleanup). Phase 73.3 COMPLETE (admin data-consumer violations fixed). Phase 103.1–103.4 COMPLETE (WCAG color contrast + heading order + duplicate landmark fixes). Phase 98 IN PROGRESS (coverage baseline: 78.18/65.94/83.01/78.51). TD-TEST-07 RESOLVED (Phase 96.6). **Phase 111.1 COMPLETE (orphan scripts deleted — lint + knip GREEN). Phase 105 COMPLETE (TD-SEC-09 RESOLVED — server-only guards on classify-task-complexity.ts + message-policy.ts). Phase 112.1 COMPLETE (test audit: 72 files classified, 51 KEEP, 19 REFACTOR, 2 REBUILD). All 7 gates GREEN.** Remaining: Phase 112.2+ TDD rebuild (core utility → server action → API route → component → E2E → coverage). | ⚠️ Testing rebuild progressing (Phases 90–112.1). **All 7 gates GREEN.** Coverage 78.18/65.94/83.01/78.51. WCAG major fixes delivered. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
24. Testing infrastructure rebuild delivered through Phase 100.3. Key phases: 90.1–90.7 (structure + tests), 91.1–91.5 (E2E), 92.1–92.2 (server-only guards), 86 (extended guards), 94.1–94.5 (config hardening), 95.1–95.4 (E2E structural assertions), 95-R (Chromium fix — Playwright workers=1 + resolvePlaywrightWorkers()), 96.1 (conversation-stop merge), 96.2 (pure utility un-mocking), 96.3 (webhook idempotency tests), 99.1 (aria-live), 99.3 (stable message keys), 100.1–100.3 (code deduplication — isObjectRecord, VALID_PERSONA_ID_SET, legalReviewDisclaimer). **72 suites, 433 tests — all pass. E2E: 108 passed, 0 failed, 25 skipped. Coverage: 78.18/65.94/83.01/78.51. Build passing. All 7 gates GREEN. Phase 111.1 COMPLETE (orphan scripts deleted). Phase 105 COMPLETE (TD-SEC-09 RESOLVED). Phase 112.1 COMPLETE (test audit).** TD-WCAG-02 RESOLVED, TD-WCAG-03 RESOLVED, TD-WCAG-04 RESOLVED, TD-REUSE-01 FULLY RESOLVED, TD-LEAK-01 RESOLVED, TD-TEST-07 RESOLVED (Phase 96.6). Phase 96.5–96.8 COMPLETE, Phase 99.2 COMPLETE (native dialog), Phase 99.4 COMPLETE (knip cleanup), Phase 73.3 COMPLETE (admin data consumers), Phase 103.1–103.4 COMPLETE (WCAG fixes). Phase 98 IN PROGRESS (coverage baseline 78.18/65.94/83.01/78.51). Remaining: **Phase 112.2+ TDD rebuild.**
...
> **Status: COMPLETED** — MongoDB-backed rate limiting (Phase 25.3), error sanitization (HF series), webhook hardening (Phases 27.8–27.10), atomic limit enforcement (Phases 27.1, 28.1, 28.6), usage event logging (Phase 16) all delivered. Operational monitoring through UsageEvent model and admin usage analytics.
...
> **Status: COMPLETED** — Validation workflow passes (Prettier, lint, tsc, 72 suites / 433 unit tests, build). E2E: 108 passed, 0 failed, 25 skipped. All six validation gates green. Release gates A–F green.
...
Run in this order and require all seven to pass:
...
**Current status (March 2026):** All 7 gates GREEN. Prettier, lint (0 errors), tsc, unit tests (72 suites / 433 tests), E2E (108 passed, 0 failed, 25 skipped), build, and knip (0 findings) all pass. Phase 111.1 COMPLETE (orphan scripts deleted). Phase 105 COMPLETE (TD-SEC-09 RESOLVED). Phase 112.1 COMPLETE (test audit). Coverage: 78.18/65.94/83.01/78.51. All prior technical debt items resolved: TD-WCAG-02/03/04, TD-REUSE-01/02/03, TD-LEAK-01, TD-TEST-07, TD-SEC-05/06/07/08, TD-MEDIA-01, TD-DATA-01/02, TD-UX-05/07, TD-DS-04/05/06.

**Note:** TDD rebuild (March 2026 mandate) will temporarily reduce test counts as existing tests are removed and rebuilt from scratch. Gate F enforcement continues — all 7 gates must pass at every commit. Phase 111.1 RESOLVED. Phase 112.2+ TDD rebuild is next.

---

## 9. Immediate Execution Order

Milestones 0–24 are COMPLETE. Milestone 25 IN PROGRESS — **TDD TESTING REBUILD IS PRIMARY FOCUS**. All Phases 1–112.1 complete (433 unit tests, 72 suites; 108 E2E passed, 0 failed, 25 skipped; coverage 78.18/65.94/83.01/78.51). **All 7 gates GREEN.** Phase 111.1, 105, 112.1 DONE. Node.js 24.12.0 runtime confirmed.
...
1. **Phase 111 (BLOCKER)** — Clean orphan dev scripts (`_update_plan.js`, `_up.js`) causing 29 lint errors + 2 knip findings. All 7 gates must be GREEN before any other work.
2. **Phase 105 (HIGH)** — Add `server-only` guards to `classify-task-complexity.ts` and `message-policy.ts` (TD-SEC-09).
3. **TDD testing rebuild (Phase 112)** — remove all existing tests, rebuild from scratch with TDD (Milestone 25 primary focus).
4. **Admin configurability closure** — Phase 106 (shared types), Phase 107 (stop-reason admin config), FAQ content (74.2), hero/about/landing copy (104).
5. **WCAG 2.2 AA** — Phases 108 (tabs a11y), 109 (icons a11y), 110 (hamburger aria).
6. **Remaining backlog** — 87 (createTaskSchema strict), low-priority items.
...
13. ✅ Milestone 22 technical debt, usage display, code quality (Phases 63–67.3, 61.1).
14. ✅ Lime Green button palette (Phases 68.1–68.3), non-terminal media stops (Phase 69.1), admin shell alignment (Phase 70.1).
15. ✅ Button text correction (Phase 68.4), admin persona configurability (Phases 71.1–71.2), admin design polish (Phase 70.2).
16. ✅ Node.js stabilization (Phase 76): repo-wide error scan, `server-only` guards, `knip` cleanup, MongoDB SRV resilience, self-heal dedupe, Next.js scroll warning fix, all six validation gates passed. Runtime upgraded to 24.12.0 (PM audit #37).
17. ✅ Dev/runtime stability: MongoDB SRV handled, scroll-behavior fixed, Clerk dev-key classified as env warnings (Phase 76).
18. ✅ `npm run knip` audit complete with item-by-item verification (Phase 76). ⚠️ 2 new findings detected (PM audit #37) — Phase 83.
19. ✅ limeGreen palette updated from #B8F60D to #D9F20C in globals.css. All 10 steps recalculated (Phase 82 COMPLETE).
20. ✅ Knip cleanup: `@typescript-eslint/parser` removed from devDeps, `chatTools` made non-exported. `npm run knip` clean (Phase 83 COMPLETE).
...
38. ✅ Server-only credential guards: `import "server-only"` added to `openai.tsx`, `aws.tsx`, `resolve-entitlements.tsx`, `download-url-allowlist.ts` (Phase 92.1 + 92.2 COMPLETE). TD-SEC-07 + TD-SEC-08 RESOLVED.
39. ✅ Coverage thresholds raised to 80/75/80/80 (Phase 93.1 COMPLETE). ESLint `react-hooks/set-state-in-effect` re-enabled at warn level (Phase 93.2 COMPLETE).
40. ✅ Extended server-only guards: 17 files guarded (8 Mongoose models, 5 OpenAI utils, 4 AWS utils). `s3-file-reference.ts` excepted (client import chain). TD-SEC-06 RESOLVED (Phase 86 COMPLETE).
41. ✅ Admin table selection clearing tests delivered (Phase 90.6 COMPLETE).
42. ✅ Admin bulk E2E, cleanup, behavioral tests delivered (Phases 91.2, 91.3, 91.4 COMPLETE).
43. ✅ WCAG admin tables: admin-users-table and admin-transactions-table converted to semantic HTML (Phase 72.4 COMPLETE).
44. ✅ Viewport zoom restriction removed from layout.tsx — WCAG SC 1.4.4 fixed (Phase 101 COMPLETE). TD-WCAG-03 RESOLVED.
45. ✅ 4th isObjectRecord duplicate removed from effective-model-config.ts (Phase 100.4 COMPLETE). TD-REUSE-01 FULLY RESOLVED.
46. ✅ AbortController added to chat streaming fetch — resource leak fixed (Phase 102 COMPLETE). TD-LEAK-01 RESOLVED.
47. ✅ Admin auth path tests: unauthenticated/forbidden/admin success (Phase 96.4 COMPLETE).
48. ✅ AudioPlayer ARIA improvements: accessible labels, progressbar semantics + ESLint fix (Phase 99.5 COMPLETE). TD-WCAG-04 RESOLVED.
49. ✅ WCAG E2E via @axe-core/playwright: 7 public routes scanned, known violations documented (Phase 97.1 COMPLETE). TD-WCAG-02 RESOLVED.
50. ✅ Knip cleanup: dead exports and unused helpers removed (Phase 99.4 COMPLETE). 0 knip findings.
51. ✅ WCAG color contrast fixed on landing page (Phase 103.1 COMPLETE).
52. ✅ WCAG color contrast fixed on plans page (Phase 103.2 COMPLETE).
53. ✅ Heading order fixed on personas page (Phase 103.3 COMPLETE).
54. ✅ Duplicate landmark violations fixed — extra `<main>` removed (Phase 103.4 COMPLETE).
55. ✅ Upload file size validation tests added (Phase 96.5 COMPLETE).
56. ✅ Zustand store tests added — 3 stores tested (Phase 96.6 COMPLETE). TD-TEST-07 RESOLVED.
57. ✅ Component tests added — confirmation-modal, plan-card, persona-card, checkout-form (Phase 96.7 COMPLETE).
58. ✅ User model tests expanded — required fields, enum validation, defaults, indexes (Phase 96.8 COMPLETE).
59. ✅ Admin client data-consumer violations fixed — sidebar, models section, personas section (Phase 73.3 COMPLETE).
60. ✅ ConfirmationModal migrated to native `<dialog>` semantics (Phase 99.2 COMPLETE).
61. ✅ Coverage baseline established: Statements 78.18%, Branches 65.94%, Functions 83.01%, Lines 78.51% (Phase 98 STARTED).
...
15. ✅ Non-terminal media-specific stops (Phase 69.1) — DONE. TD-UX-07 RESOLVED. Media limit hits return `endAction: "start_new_conversation"` with `taskStatus: "active"`. Client distinguishes terminal vs non-terminal stops.
...
| 3   | Components must be data consumers                            | CRITICAL | ✅ COMPLETE (Phase 73.1 + 73.3). All 8 components converted. Admin sidebar, models, personas sections fixed (Phase 73.3).                                                                 |
| 4   | Reduce unnecessary renders and resource leaks                | CRITICAL | ✅ COMPLETE. All useEffect cleanups verified. AbortController added to chat streaming (Phase 102). TD-LEAK-01 RESOLVED.                                                                   |
...
| 8   | WCAG 2.2 AA accessibility compliance                         | HIGH     | ⚠️ MOSTLY COMPLETE. 72.1–72.4 ✅ Phase 101 ✅ 99.5 ✅ 97.1 ✅ 103.1–103.4 ✅. Remaining: full audit pass.                                                                                 |
| 9   | npm run knip — unused code audit                             | HIGH     | ⚠️ **REGRESSED (PM audit #48).** 2 findings: `_up.js`, `_update_plan.js` (orphan dev scripts in project root). Phase 111 BLOCKER tracks fix.                                              |
...
| 14  | RE-USE repetitive code as much as possible                   | HIGH     | ⚠️ COMPLETE. Phase 100.1–100.4 resolved all duplicate patterns. TD-REUSE-01/02/03 ALL RESOLVED.                                                                                           |

**Owner Directive Updates (March 2026 — Palette & Button Styles):**

| #   | Directive                                                                    | Priority | Status                                                                                                                                                                              |
| --- | ---------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 12  | Brand color palette: add Lime Green (#D9F20C) as NEW accent                  | HIGH     | ✅ COMPLETE (Phase 68.1 + 82). limeGreen palette 100–1000 added to theme tokens. Applied across button styles. Base hex #D9F20C with all 10 steps recalculated (Phase 82 COMPLETE). |
| 13  | Button styles: `.btn-text`, `.btn-outlined`, `.btn-contained` use Lime Green | HIGH     | ✅ COMPLETE (Phase 68.2–68.4). .btn-contained text = Midnight Blue (500). .btn-text hover = Lime Green (800).                                                                       |
| 14  | Theme definitions: Text (Primary) / BG (Secondary) naming                    | HIGH     | ✅ IMPLEMENTED (Phase 68.1). Theme tokens operational. Light: Text = Midnight Blue, BG = Lavender Haze. Dark: Text = Lavender Haze, BG = Night Indigo.                              |
| 15  | Admin panel layout — owner reports still old design                          | HIGH     | ✅ COMPLETE (Phase 70.1 + 70.2). Full admin design alignment delivered.                                                                                                             |
...
| 17  | `.btn-contained` text color correction: Midnight Blue (500)                  | HIGH     | ✅ COMPLETE (Phase 68.4). Corrected to Midnight Blue (500) for both default and hover.                                                                                              |
| 18  | Admin configurability still incomplete                                       | HIGH     | ✅ COMPLETE. Core propagation + persona content editing (71.1 + 71.2). Remaining v2: system prompts, hero images.                                                                   |
| 19  | Admin panel design still needs further work                                  | HIGH     | ✅ COMPLETE (Phase 70.2). Full design alignment pass delivered.                                                                                                                     |
...
**Newly completed (Milestone 23 Block A, verified 2026-03-19):**

18. ✅ Button text color correction (Phase 68.4) — DONE. .btn-contained text = Midnight Blue (500). .btn-text hover = Lime Green (800).
19. ✅ Admin persona content configurability (Phase 71.1) — DONE. `getEffectivePersonaConfig()` utility with AppSetting overrides.
...
**Current priority (Milestone 25 — March 2026 TDD Rebuild):**
...
- ✅ Phase 94: Testing config & toolchain hardening.
- ✅ Phase 95 + 95-R: Fix 4 failing E2E specs + Chromium resolution.
- ✅ Phase 96.1–96.8: Unit test quality rebuild (conversation-stop, un-mocking, webhook, admin auth, upload, Zustand, components, User model).
- ✅ Phase 97.1: WCAG E2E via axe-core (7 public routes).
- ✅ Phase 72.4: WCAG table semantics.
- ✅ Phase 73.3: Admin client data-consumer violations fixed.
- ✅ Phase 99.1–99.5: WCAG quick fixes + code quality.
- ✅ Phase 100.1–100.4: Code deduplication.
...
> **All Milestones 0–24 COMPLETE. All Phases 1–103.4 complete. 433 unit tests (72 suites) passing. 108 E2E passed, 0 failed, 25 skipped. Build passing. Node.js 24.12.0 runtime confirmed. Lint FAILS (29 errors from orphan `_update_plan.js`) and knip FAILS (2 findings: `_up.js`, `_update_plan.js`) — Phase 111 BLOCKER. 5 of 7 gates GREEN. Coverage 78.18/65.94/83.01/78.51. Milestone 25 IN PROGRESS — PRIMARY FOCUS: Phase 111 gate fix → TDD testing rebuild from scratch (March 2026 owner mandate). Previous incremental testing phases (97.2+, 98) superseded by full TDD rebuild.**
...
| #   | Directive                                                                      | Priority | Status                                                                                                                                                                                                                   |
| --- | ------------------------------------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1  | DEEP check of all techstack config and setup                                   | CRITICAL | ✅ COMPLETE (Phase 89 + PM audit #42 triple-audit). ESLint, deps, vitest, Playwright all hardened. Remaining hardening in Phase 94.                                                                                      |
| D2  | DEEP check on Playwright configs and all testing infrastructure for misconfigs | CRITICAL | ✅ COMPLETE (Phase 89.4 + PM audit #42). 3 browsers, full matrix env flag. Remaining: actionTimeout (Phase 94.2).                                                                                                        |
| D3  | DEEP analysis of entire unit testing flow — AGGRESSIVE testing process         | CRITICAL | ⚠️ PARTIALLY DELIVERED. 90.1–90.3, 90.6, 90.7 DONE. Coverage gate NOT met. Over-mocking epidemic. Remaining: Phase 96.                                                                                                   |
| D4  | DEEP analysis of entire e2e testing flow — AGGRESSIVE testing process          | CRITICAL | ✅ E2E FIXED (Phase 95-R). 108 passed, 0 failed, 25 skipped. Remaining: Phase 97 (WCAG axe-core E2E).                                                                                                                    |
| D5  | Refactor all unit and e2e tests from scratch; rebuild entire testing process   | CRITICAL | ⚠️ **TDD REBUILD MANDATED (March 2026).** Previous incremental work (Phases 88–103.4) complete. Owner now mandates FULL removal and rebuild of ALL tests from scratch using strict TDD. Supersedes 97.2+ and 98.         |
...
| D8  | RE-USE repetitive code as much as possible                                     | HIGH     | ✅ COMPLETE. Phase 100.1–100.4 resolved all 4 patterns. TD-REUSE-01/02/03 ALL RESOLVED.                                                                                                                                  |
| D9  | WCAG 2.2 AA accessibility compliance                                           | HIGH     | ⚠️ MOSTLY COMPLETE. 72.1–72.4 ✅. Phase 101 ✅ (viewport). Phase 99.5 ✅ (AudioPlayer). Phase 97.1 ✅ (WCAG E2E). Phase 103.1–103.4 ✅ (color contrast, heading order, duplicate landmarks). Remaining: full audit pass. |
| D10 | Components must be data consumers                                              | CRITICAL | ✅ COMPLETE (Phase 73.1 + 73.3). All components converted. Admin data-consumer violations fixed (Phase 73.3).                                                                                                            |
| D11 | Good practices — reduce unnecessary renders and resource leaks                 | CRITICAL | ⚠️ COMPLETE. All useEffect cleanups verified. AbortController added to chat streaming (Phase 102). TD-LEAK-01 RESOLVED.                                                                                                  |
...
| D14 | Clean unused via npm run knip                                                  | HIGH     | ⚠️ **REGRESSED (PM audit #48).** 2 findings: orphan scripts `_up.js`, `_update_plan.js`. Phase 111 BLOCKER.                                                                                                              |
...
| 2   | RE-USE repetitive code as much as possible                                     | HIGH     | ✅ COMPLETE. Phase 100.1–100.4 resolved all 4 patterns. TD-REUSE-01/02/03 ALL RESOLVED.                                                                                                                         |
| 3   | WCAG 2.2 AA accessibility compliance for entire app                            | HIGH     | ⚠️ MOSTLY COMPLETE. 72.1–72.4 ✅. Phase 101 ✅ (viewport). Phase 99.5 ✅ (AudioPlayer). Phase 97.1 ✅ (WCAG E2E). Phase 103.1–103.4 ✅ (color contrast, heading order, landmarks). Remaining: full audit.       |
| 4   | Components must be data consumers (especially `"use client"`)                  | CRITICAL | ✅ COMPLETE (Phase 73.1 + 73.3). All 8 components converted including admin data-consumer violations.                                                                                                           |
| 5   | Good practices — reduce unnecessary renders and resource leaks                 | CRITICAL | ⚠️ COMPLETE. AbortController added (Phase 102). All useEffect cleanups verified. TD-LEAK-01 RESOLVED.                                                                                                           |
...
| 8   | `npm run knip` — unused code audit workflow                                    | HIGH     | ⚠️ **REGRESSED (PM audit #48).** 2 findings: orphan scripts `_up.js`, `_update_plan.js`. Phase 111 BLOCKER.                                                                                                     |
...
| 11  | DEEP techstack config check, Playwright/Vitest config audit                    | CRITICAL | ✅ COMPLETE (Phase 89 + 94). All config hardening delivered.                                                                                                                                                    |
| 12  | Full testing infrastructure rebuild — TDD approach, aggressive testing         | CRITICAL | ⚠️ **TDD REBUILD MANDATED (March 2026).** Previous phases (86–103.4) complete. Full TDD rebuild from scratch is Milestone 25 primary deliverable. Phase 111 BLOCKER must be resolved first. 5 of 7 gates GREEN. |
...
| 14  | RE-USE repetitive code — extract duplicated patterns                           | HIGH     | ✅ COMPLETE. TD-REUSE-01/02/03 ALL RESOLVED (Phase 100.1–100.4).                                                                                                                                                |

### Owner Instructions (PM Audit #48 — Canonical Tracking)

These are the **10 key owner instructions** that must be tracked and enforced across all implementation decisions. Updated 2026-03-23.

| #    | Owner Instruction                                                          | Priority | Tracking Status                                                                                                                                                                                                           |
| ---- | -------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OI1  | Refactor ALL unit and e2e tests from scratch — TDD methodology             | CRITICAL | ⚠️ **TDD REBUILD MANDATED (March 2026).** Remove ALL existing tests and rebuild from scratch. Previous 433 tests (72 suites) + 108 E2E are baseline reference only. Full TDD rebuild is Milestone 25 primary deliverable. |
| OI2  | NO HARDCODED data — everything MUST be fully configurable from ADMIN panel | CRITICAL | ⚠️ PARTIALLY DELIVERED. Core done (74.1). Remaining: FAQ (74.2), landing copy (74.3).                                                                                                                                     |
| OI3  | RE-USE repetitive code as much as possible                                 | HIGH     | ✅ COMPLETE. Phase 100.1–100.4 resolved all patterns. TD-REUSE-01/02/03 ALL RESOLVED.                                                                                                                                     |
| OI4  | WCAG 2.2 AA compliance                                                     | HIGH     | ⚠️ MOSTLY COMPLETE. Phases 72.1–72.4, 97.1, 99.5, 101, 103.1–103.4 all delivered. Remaining: full audit pass.                                                                                                             |
| OI5  | Components must be data consumers                                          | CRITICAL | ✅ COMPLETE (Phase 73.1 + 73.3). All 8 components converted.                                                                                                                                                              |
| OI6  | Reduce unnecessary renders and resource leaks                              | CRITICAL | ✅ COMPLETE. AbortController (Phase 102). All useEffect cleanups verified. TD-LEAK-01 RESOLVED.                                                                                                                           |
| OI7  | Server-side utilities and data fetching                                    | CRITICAL | ✅ COMPLETE. Server-only guards on all utility files.                                                                                                                                                                     |
| OI8  | User removal cascades Clerk + DB + all data                                | CRITICAL | ✅ COMPLETE. All 3 paths: Clerk → Tasks → Transactions → UsageEvents → S3 → User.                                                                                                                                         |
| OI9  | Clean unused (knip)                                                        | HIGH     | ⚠️ **REGRESSED (PM audit #48).** 2 findings: orphan dev scripts. Phase 111 BLOCKER.                                                                                                                                       |
| OI10 | Admin plans/prices/features/settings fully configurable                    | HIGH     | ⚠️ PARTIALLY DONE. Core operational (74.1). Remaining: FAQ (74.2), hero/about/landing copy (74.3).                                                                                                                        |
...
> 382 unit tests passing, 180 E2E passing, build passing.
...
> Verification: Prettier pass, lint pass, tsc pass, 65 test suites / 365 unit tests passing, 174 E2E passing (6 failed — timeout/navigation in existing specs, not video-related; 48 skipped), build pass.
...
> **Status: COMPLETED** — Blocks A–C delivered (Phase 38 + 36). Block D partially delivered (33.8 DONE).
...
**Block D — Test Coverage: COMPLETED**
...
> **Status: COMPLETED** — Phase 60 delivered. 58 files migrated. Zero legacy palette references. Full validation gateway passed.

**Delivered:**

1. Color palette upgrade: Navy (#0D3B66), Lemon (#FAF0CA), Grass (#27A148) — applied across all surfaces.
2. Removed custom `lightAccent`, `darkAccent`, `jwdAqua`, `jwdMarine` palettes — zero references remain.
3. Removed custom `lightBorders`, `darkBorders` palettes — replaced with TailwindCSS `slate`.
4. All `bg-white` instances replaced with design tokens (zero remain in src/).
5. Text tokens migrated: lightText to Navy, darkText to Lemon.
6. Accent tokens unified: both themes use Grass (#27A148).

**Remaining items moved to Phase 61/63:** Admin design alignment, JSON outsourcing, admin users table columns, document archiving.
...
| **Lime Green**  | `#D9F20C` | **NEW accent color** (buttons, CTAs) in both themes |
...
> **Note:** Lime Green (#D9F20C) is the NEW accent color used for button interactions across both themes. Previous accent colors (Twilight Purple for light, Dusty Blue for dark) are superseded for button styling by Lime Green.
...
### Milestone 23 — Owner Corrections, Accessibility & Codebase Quality (2026-03-19)

> **Status: IN PROGRESS** — Block A (Phases 68.4 + 71.1 + 71.2 + 70.2) COMPLETE and archived as delivered code. Block B partially delivered: Phase 72.1 COMPLETE, Phase 74.1 COMPLETE. Phases 72.2–72.4, 74.2–74.3, 73.3, 75 remain OPEN, and several "delivered" owner-facing items now require explicit re-verification in Milestone 24 before closeout.

**Objective:** Apply owner-directed corrections, achieve WCAG 2.2 AA compliance, audit codebase for good practices, eliminate remaining hardcoded data, and clean /faqs route references from documentation.

**Dependencies:** Milestone 22.

**Block A — Owner Corrections (COMPLETE):**

1. ✅ **HIGH** — Phase 68.4: Fix `.btn-contained` text color to Midnight Blue (500). Fix `.btn-text` hover to Lime Green (800). DONE.
2. ✅ **HIGH** — Phase 71.1: Create `getEffectivePersonaConfig()` utility with AppSetting override layer for persona content. DONE.
3. ✅ **HIGH** — Phase 71.2: Add persona content editing to admin settings (labels, taglines, descriptions, starter prompts). DONE.
4. ✅ **HIGH** — Phase 70.2: Full admin panel design alignment with `/app` design system. DONE.

**Block B — Owner-Directed New Work (OPEN):**

5. **HIGH** — Phase 72: WCAG 2.2 AA accessibility compliance pass. Phase 72.1 COMPLETE (admin skip-link, main landmark, aria-current). Phase 72.1 COMPLETE (admin skip-link, main landmark, aria-current). Audit all pages (public + app + admin) for: semantic HTML, keyboard navigation, focus management, color contrast ratios, ARIA labels, screen reader compatibility, form accessibility, error identification. Fix all AA violations.

6. **CRITICAL** — Phase 73: Codebase good practices audit. Systematic review for:
   - Unnecessary re-renders in client components (memo, useMemo, useCallback where justified).
   - Resource leaks (uncleaned useEffect subscriptions, event listeners, timers).
   ...
   - Ensure all `"use client"` components are pure data consumers (props from Server Components, no fetch).

7. **CRITICAL** — Phase 74: Full admin configurability — zero hardcoded data references. Phase 74.1 COMPLETE (support email). Phase 74.1 COMPLETE (support email). Audit for:
   - Any remaining hardcoded plan rules, prices, limits, or feature flags outside admin settings + fallback constants.
   - Any component directly importing constants that should resolve from admin settings.
   - Verify: admin change → runtime behavior change for ALL configurable surfaces.

8. **LOW** — Phase 75: /faqs route removal from all documentation and navigation references. Clean up:
   - AGENTS.md route table and project structure
   - SPEC.md route table and descriptions
   - ThePlan.md historical references (add “removed” annotations, do not delete history)
   - Any remaining `/faqs` href links in source code (verified: none in src/ currently)

**Success Criteria:**

- Block A: All 4 phases verified COMPLETE.
- Phase 72: Zero WCAG 2.2 AA violations on all screens.
- Phase 73: Zero unnecessary re-renders, zero resource leaks, zero client-side fetching.
- Phase 74: Zero hardcoded data references outside admin settings + constants fallbacks.
- Phase 75: No active /faqs route references in docs or code.
...
**Dependencies:** Milestone 23 Block B.

**Block A — Node.js 24.12.0 Hardening & Repo-Wide Scan (CRITICAL):**

1. ✅ **COMPLETE** — Phase 76: Repo-wide codebase scan, compile/runtime regression fixes, all six validation gates passed.
2. ✅ **COMPLETE** — Phase 77.1: MongoDB SRV-failure resilience added with fallback URI support.
3. ✅ **COMPLETE** — Phase 77.2: Next.js scroll-behavior warning resolved.
4. ✅ **COMPLETE** — Phase 77.3: Clerk development-key messages classified as environment warnings.

**Block B — `knip` Unused-Code Audit (HIGH):**

5. **HIGH** — Phase 78: Run `npm run knip`, review every reported dependency/export/file individually, and decide one of two outcomes only:
   - remove it because it is truly unused, or
   - keep it and add a narrow, justified `knip.json` ignore because the usage is indirect or tool-driven.
6. **HIGH** — No bulk deletions and no blanket ignore patterns. `knip` output is a lead list, not proof.

**Block C — Owner-Facing Regression Re-Verification (HIGH):**

7. **HIGH** — Phase 79.1: Re-verify the Premium media-limit path against current code. Confirm that hitting the video quota does not show a generic all-media failure and does not incorrectly terminate otherwise-usable chat.
8. **HIGH** — Phase 79.2: Re-verify `/app/profile` shows current plan limitations and usage correctly for the active user and matches resolved entitlements.
9. **HIGH** — Phase 79.3: Re-verify `/admin/users` table and single-user detail show included versus remaining usage clearly and without placeholders.
10. **HIGH** — Phase 79.4: Re-check the owner-reported admin-configurability gap and list any remaining settings/content still hardcoded beyond approved constant fallbacks.

**Success Criteria:**

- Node 24.12.0 hardening is proven by all six validation gates, not just package metadata.
- The root cause and resolution path for the `querySrv ECONNREFUSED` error are documented and reproducible.
- The Next scroll-behavior warning is either resolved or intentionally accepted with explicit rationale.
...
> **Status: IN PROGRESS** — PM audit #42 triple-audit. Owner directive: full testing infrastructure rebuild from scratch. TDD approach mandated. Phases 88–93 COMPLETE. New phases 94–98 defined for remaining testing rebuild (config hardening, 4 failing E2E fixes, unit test quality, E2E quality, coverage gate).

**Objective:** Fix P1 stale-selection bugs in admin tables, harden all tech stack configurations, rebuild the entire testing infrastructure from scratch with aggressive behavioral coverage and TDD discipline, and achieve coverage gate enforcement.

**Dependencies:** Milestones 0–24 (all complete).

**Block A — P1 Bug Fixes (CRITICAL): COMPLETE**

1. ✅ ~~**CRITICAL** — Phase 88.1: Fix stale `selectedUserIds` in `admin-users-table.tsx`.~~ DONE.
2. ✅ ~~**CRITICAL** — Phase 88.2: Fix stale `selectedTransactionIds` in `admin-transactions-table.tsx`.~~ DONE.

**Block B — Config Hardening (CRITICAL): COMPLETE**

3. ✅ ~~**CRITICAL** — Phase 89.1: Delete dead `.eslintrc.json`.~~ DONE.
4. ✅ ~~**CRITICAL** — Phase 89.2: Move runtime deps from devDependencies to dependencies.~~ DONE.
5. ✅ ~~**HIGH** — Phase 89.3: Add `environmentMatchGlobs` to vitest config.~~ DONE.
6. ✅ ~~**MEDIUM** — Phase 89.4: Reduce Playwright from 7 to 3 browser projects.~~ DONE.

**Block C — Unit Test Rebuild (HIGH): MOSTLY COMPLETE**

7. ✅ ~~**HIGH** — Phase 90.1: Create unit test directory structure.~~ DONE.
8. ✅ ~~**HIGH** — Phase 90.2: Consolidate 5 duplicate test files.~~ DONE.
9. ✅ ~~**HIGH** — Phase 90.3: Add failure path tests for media generation (TDD).~~ DONE.
10. ✅ ~~**MEDIUM** — Phase 90.6: Admin table selection clearing tests (TDD).~~ DONE.
11. ✅ ~~**MEDIUM** — Phase 90.7: Security injection tests for validation schemas (TDD).~~ DONE.

**Block D — E2E Test Rebuild (HIGH): MOSTLY COMPLETE**

12. ✅ ~~**HIGH** — Phase 91.1: Consolidate overlapping public page specs.~~ DONE.
13. ✅ ~~**HIGH** — Phase 91.2: Admin bulk actions + pagination selection reset E2E.~~ DONE.
14. ✅ ~~**HIGH** — Phase 91.3: Add cleanup to admin-features.spec.ts.~~ DONE.
15. ✅ ~~**MEDIUM** — Phase 91.4: Convert admin-users.spec.ts from smoke to behavioral.~~ DONE.
16. ✅ ~~**MEDIUM** — Phase 91.5: Error state E2E testing.~~ DONE.

**Block E — Security Guards (CRITICAL): COMPLETE**

17. ✅ ~~**CRITICAL** — Phase 92.1: `import "server-only"` on openai.tsx + aws.tsx.~~ DONE.
18. ✅ ~~**CRITICAL** — Phase 92.2: `import "server-only"` on resolve-entitlements + download-url-allowlist.~~ DONE.
19. ✅ ~~**MEDIUM** — Phase 86: Extended server-only guards on 17 files.~~ DONE.

**Block F — Coverage & Config Tightening (HIGH): PARTIALLY COMPLETE**

20. ✅ ~~**HIGH** — Phase 93.1: Raise Vitest coverage thresholds to 80/75/80/80.~~ DONE.
21. ✅ ~~**MEDIUM** — Phase 93.2: Re-enable `react-hooks/set-state-in-effect` ESLint rule.~~ DONE.

**Block G — Testing Infrastructure Rebuild (PM audit #42 + #44): IN PROGRESS**

22. **CRITICAL** — Phase 94: Testing config & toolchain hardening — **DONE** (94.1–94.5 complete).
23. **CRITICAL** — Phase 95: Fix 4 failing E2E specs — **DONE** (95.1–95.4 complete). ✅ Phase 95-R: Chromium failures RESOLVED (Playwright workers=1, resolvePlaywrightWorkers()).
24. **HIGH** — Phase 96: Unit test quality rebuild — **96.1–96.8 ALL DONE**. Upload validation (96.5), Zustand stores (96.6), component tests (96.7), User model (96.8).
25. **HIGH** — Phase 97: E2E quality rebuild — **97.1 DONE** (WCAG via axe-core, 7 public routes). Remaining: admin propagation, Mongo dedup.
26. **HIGH** — Phase 98: Coverage gate — **IN PROGRESS**. Baseline established: 78.18/65.94/83.01/78.51.
27. **HIGH** — Phase 99: WCAG quick fixes + code quality — **ALL DONE**. 99.1 (aria-live), 99.2 (native dialog), 99.3 (stable keys), 99.4 (knip cleanup), 99.5 (AudioPlayer ARIA).
28. **HIGH** — Phase 100: Code deduplication. ~~100.1–100.4 ALL DONE.~~ TD-REUSE-01 FULLY RESOLVED.
29. ~~**CRITICAL** — Phase 101: Remove viewport zoom restriction.~~ **DONE.** TD-WCAG-03 RESOLVED.
30. ~~**HIGH** — Phase 102: Add AbortController to chat streaming fetch.~~ **DONE.** TD-LEAK-01 RESOLVED.
31. ~~**CRITICAL** — Phase 73.3: Fix admin client data-consumer violations (sidebar, models, personas).~~ **DONE.**
32. ~~**HIGH** — Phase 103.1: WCAG color contrast fix on landing page.~~ **DONE.**
33. ~~**HIGH** — Phase 103.2: WCAG color contrast fix on plans page.~~ **DONE.**
34. ~~**HIGH** — Phase 103.3: Heading order fix on personas page.~~ **DONE.**
35. ~~**HIGH** — Phase 103.4: Duplicate landmark violations fixed (extra `<main>` removed).~~ **DONE.**

**Success Criteria:**

- ✅ P1 stale selection bug eliminated — Phase 88 DONE.
- ✅ Config hardening complete — Phase 89 DONE.
- ✅ Server-only credential guards — Phases 86, 92 DONE.
- ✅ Coverage thresholds raised + ESLint rule re-enabled — Phase 93 DONE.
- ✅ Testing config & toolchain hardening — Phase 94 DONE (thresholds, factories, ESLint, Playwright).
- ✅ 4 failing E2E specs fixed with structural assertions — Phase 95 DONE.
- ✅ E2E Chromium failures resolved — Phase 95-R DONE (workers=1, resolvePlaywrightWorkers()).
- ✅ Conversation-stop duplicate merge + mock scope reduction — Phase 96.1–96.2 DONE.
- ✅ Webhook idempotency and edge-case tests — Phase 96.3 DONE.
- ✅ WCAG aria-live + stable message keys — Phase 99.1 + 99.3 DONE.
- ✅ Code deduplication (3 of 4 patterns resolved) — Phase 100.1–100.3 DONE.
- ✅ 433 unit tests (72 suites). E2E 108/0/25. All 6 gates GREEN. Coverage: 78.18/65.94/83.01/78.51.
- ✅ Viewport zoom WCAG fix — Phase 101 DONE. WCAG SC 1.4.4 fixed.
- ✅ 4th isObjectRecord dedup — Phase 100.4 DONE. TD-REUSE-01 FULLY RESOLVED.
- ✅ AbortController resource leak — Phase 102 DONE. TD-LEAK-01 RESOLVED.
- ✅ AudioPlayer ARIA + ESLint — Phase 99.5 DONE. TD-WCAG-04 RESOLVED.
- ✅ Admin auth path tests — Phase 96.4 DONE.
- ✅ Upload/Zustand/component/model tests — Phase 96.5–96.8 DONE. TD-TEST-07 RESOLVED.
- ✅ WCAG E2E tests via axe-core — Phase 97.1 DONE (7 public routes). TD-WCAG-02 RESOLVED.
- ⬜ Remaining E2E quality — Phase 97.2+ PENDING.
- ⬜ Coverage gate GREEN (branches ≥78%, statements ≥82%) — Phase 98 IN PROGRESS. Baseline: 78.18/65.94/83.01/78.51.
- ✅ Knip cleanup — Phase 99.4 DONE. 0 findings.
- ✅ ConfirmationModal native dialog — Phase 99.2 DONE.
- ✅ Admin data-consumer violations — Phase 73.3 DONE.
- ✅ WCAG color contrast, heading order, landmarks — Phase 103.1–103.4 DONE.
...
12. Stop treating "implemented in code" as equivalent to "verified in owner-facing runtime behavior."
13. Stop using `knip` output as an automatic deletion script; every item requires proof.
