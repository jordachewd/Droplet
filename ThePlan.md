...
| Test infrastructure      | **TDD REBUILD IN PROGRESS (Phase 120.2).** 76 unit test suites, 480 tests (all pass). E2E: 14 spec files, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN. TDD infrastructure built (Phase 120.1). Phase 120.2 Batch A+B COMPLETE (14 utility files). 34 utility test files rebuilt out of ~40 target. Previous 532-test suite deleted per owner directive; rebuild from scratch underway. Phases 121–126 COMPLETE. 200+ `as never` casts remain in 12+ non-rebuilt test files. Coverage: stmts 85.47, branches 72.36, funcs 89.5, lines 85.68. | ✅ All 7 gates GREEN. 480 tests (76 suites). E2E: 14 specs, 108 passed, 0 failed, 25 skipped. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
24. Testing infrastructure rebuild delivered through Phase 120.1 + 117 + 113.2 + 112.2 + 109 + 115 + 116. **TDD REBUILD IN PROGRESS.** Previous 532-test suite deleted. Current: **76 suites, 480 tests — all pass. E2E: 14 specs, 108 passed, 0 failed, 25 skipped. Build passing. All 7 gates GREEN.** Phase 120.2 Batch A+B COMPLETE (14 utility files hardened, 409→480). 34 of ~40 utility test files rebuilt. Phases 121–126 COMPLETE. **Owner directive: FULL TDD REBUILD from scratch.**
25. Owner directives (March 2026 update — ESCALATED): **TDD REBUILD MANDATED.** Owner directive: remove ALL existing unit and E2E tests and rebuild entire testing process from scratch using strict TDD methodology. Previous incremental rebuild (Phases 88–103.4) is complete and archived. Previous 532-test suite has been deleted. Current 480 tests (76 suites) and 14 E2E spec files represent the partially-rebuilt baseline. Additional mandates: NO hardcoded data (everything admin-configurable), WCAG 2.2 AA full compliance, components as data consumers, code reuse maximized, server-side utilities, user removal cascade verified, knip stays clean, reduce unnecessary renders/leaks.
26. Owner directives (March 2026 — FINAL, consolidated): (a) **TDD rebuild — CRITICAL: IN PROGRESS. Old tests deleted. 480 tests (76 suites) rebuilt so far. Phase 120.2 Batch A+B COMPLETE. 34/~40 utility files rebuilt.** (b) Full admin configurability NON-NEGOTIABLE — NO HARDCODED data, everything admin-configurable. Remaining: FAQ (74.2), landing copy (104), stop msgs (107). (c) WCAG 2.2 AA compliance — major fixes done (Phases 72, 97.1, 99.5, 101, 103), full audit pass remaining. (d) `npm run knip` must stay clean — CLEAN (0 findings, Phase 122 resolved orphan). (e) `/app/profile` plan limitations display — DELIVERED (Phase 66.1). (f) Code reuse maximized — TD-REUSE-01/02/03/05 ALL RESOLVED. Remaining: TD-REUSE-04 (Phase 106). (g) Client components must be pure data consumers — COMPLETE (Phase 73.1 + 73.3). (h) Reduce resource leaks — COMPLETE (Phase 102, TD-LEAK-01 RESOLVED). (i) User removal cascades — COMPLETE. (j) Components consume data via props — COMPLETE. (k) Reduce unnecessary renders — COMPLETE. (l) Admin configurability — core done, FAQ/landing/stop-reasons remaining.
27. PM audit #60 architect findings (July 2025): AppSetting Mongoose `category` enum missing "trial" (Zod has it — Phase 127 CRITICAL). `getAllTransactions()` unbounded query (no `.select()`/`.limit()` — Phase 125.2 HIGH). `handleError` propagates raw `error.message` to callers (Phase 126.1 HIGH). 200+ `as never` casts remain in 12+ non-rebuilt test files. No shared `<Button>` component exists (30+ raw button elements — Phase 128 HIGH). 3 Mongoose models missing explicit `strict: true` on `findOneAndUpdate` calls.
...
| F    | Validation Gate   | GREEN  | All 7 gates GREEN. Lint (0 errors, 6 warnings), knip (0 findings), TSC clean, unit (480/76 suites), E2E (14 specs, 108 passed, 25 skipped), build passes. |
...
> **All 7 gates GREEN.** 76 unit test suites, 480 tests (all pass). E2E: 14 specs, 108 passed, 0 failed, 25 skipped.
...
> **Phase 125 COMPLETE** — Merge conflicts round 2 resolved (52 files from failed `git merge devel`: AGENTS.md, .gitignore, agent configs, 46 zod/zustand/breakdown-test skill files, memory files).

**Priority order (PM audit #57):**

1. **Phase 120.2 CRITICAL** — TDD utility test rebuild from scratch (owner directive — continue, 34/~40 done, Batch A+B COMPLETE, Batch C: 6 remaining)
...
| ---- | ------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OI1  | TDD rebuild ALL tests from scratch                | CRITICAL | IN PROGRESS. Old tests deleted, rebuild underway. Current: 480 tests (76 suites). Phase 120.1 COMPLETE (TDD infra). Phase 120.2 Batch A+B COMPLETE (14 utility files, 409→480). 34/~40 utility files rebuilt. Phases 121–126 COMPLETE. 200+ `as never` casts remain (target: zero). |
...
> **Status: IN PROGRESS** — TDD rebuild underway. Previous 532-test suite deleted per owner directive. Current: 76 suites, 480 tests. Phase 120.2 Batch A+B complete; Batch C in progress (34/~40 utility test files rebuilt, 6 remaining). Phases 121–126 COMPLETE. PM audit #60: AppSetting enum mismatch found (Phase 127), shared Button missing (Phase 128).
...
- ⬬ TDD utility test rebuild from scratch — Phase 120.2 IN PROGRESS (26/~40 files).
