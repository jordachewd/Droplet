...
**Production deployment has occurred but bugs remain (PM audit #79, 2026-04-01):**

1. ~~**Audio playback error**~~ — **RESOLVED (Phase 168 CODE-COMPLETE, archived PM audit #79).** SSE `controllerClosed` guard, download Range support, audio player lifecycle hardening — all implemented.
2. **Stream error on media generation** — **MITIGATED (Phase 160.2).** Proactive 55s timeout safety net implemented. Client receives graceful warning instead of raw error. Root cause persists: Vercel Hobby 60s function timeout. Full fix requires Vercel Pro upgrade ($20/mo) or architecture change.
3. ~~**Payment not registering**~~ — **RESOLVED (PM audit #78.1).** Stripe webhook ops config verified. Payment flow working.
4. ~~**ToggleTheme hydration mismatch**~~ — **RESOLVED (Phase 169, PM audit #80).** `getInitialMode()` always returns `"system"`. localStorage synced via `useEffect` after mount. WCAG 2.2 AA `aria-checked` regression resolved.
5. ~~**Script tag warning in root layout**~~ — **RESOLVED (Phase 170, PM audit #80).** Raw `<script dangerouslySetInnerHTML>` in `<head>`. Zero React warnings.

The execution order is: **Fix 2 failing unit tests (Phase 171) → `cellesseon` → `droplet` rename (Phase 172) → Remaining catch blocks (Phase 167.2) → Admin promo text (Phase 162) → Global error boundary (Phase 163) → Env hardening (Phase 143) → Performance (Phase 144 config cache) → Remaining backlog.**
...
| Stream error on media gen | **MITIGATED (PM audit #80).** Phase 160.2 proactive 55s timeout safety net implemented. Sends graceful error before Vercel kills function. Client shows warning (amber) instead of raw "stream ended unexpectedly". Root cause persists: Vercel Hobby 60s function timeout. Image/audio borderline. Video always exceeds. **Requires: (1) Vercel Pro upgrade ($20/mo, 300s limit), or (2) long-term: decouple media gen from SSE response.** | **🟡 MITIGATED — Phase 160.2 safety net active. Root cause: Vercel Hobby 60s timeout.** | `src/app/api/openai/route.tsx`, `src/components/chat/chat-wrapper.tsx` |
...
| Payment checkout fix | **CRITICAL production bug (RE-OPENED PM audit #78).** Owner confirms: payment processes via Stripe but no Transaction created and no User plan updated post-deployment. Code logic verified correct by triple-audit (Architect + Engineer + PM). Webhook signature verification, Zod `.strip()`, idempotency repair, error logging — all sound. **Root cause confirmed: Stripe webhook endpoint misconfiguration.** Must verify in Stripe Dashboard: (1) endpoint URL = `https://<domain>/api/webhooks/stripe`, (2) signing secret matches `STRIPE_WEBHOOK_SECRET` env var, (3) `checkout.session.completed` event type selected, (4) check Recent Deliveries for failed attempts. **This is a zero-code ops fix.** | **🔴 RE-OPENED — requires Stripe Dashboard ops verification. Zero code changes needed.** | `src/app/api/webhooks/stripe/route.tsx`, `src/lib/actions/transaction.action.tsx` |
| Audio player controller error | ~~**CRITICAL production bug (PM audit #78, 2026-03-30).**~~ **RESOLVED (Phase 168 CODE-COMPLETE, verified PM audit #79, 2026-04-01).** All three paths implemented: (A) `controllerClosed` boolean flag checked by `emitHeartbeat()` before enqueuing, set `true` before `controller.close()`. (B) Download route `/api/download` has `parseByteRangeHeader()`, `Accept-Ranges: bytes` header, and `206 Partial Content` for Range requests. (C) Audio player `previousAudioUrlRef` reset in cleanup, `src=""` disposal, error event listener with user-facing error state. **Archived to DONE.md.** | ✅ RESOLVED (Phase 168 CODE-COMPLETE) | `src/components/shared/audio-player.tsx`, `src/app/api/download/route.tsx`, `src/app/api/openai/route.tsx` |
| ToggleTheme hydration mismatch | ~~**NEW CRITICAL bug (owner-reported, PM audit #79).**~~ **RESOLVED (Phase 169, PM audit #80).** `getInitialMode()` now always returns `"system"`, localStorage synced via `useEffect` after mount. Same pattern as Phase 153. WCAG 2.2 AA `aria-checked` regression resolved. | ✅ RESOLVED (Phase 169) | `src/components/layout/droplet-theme.tsx` |
| Script tag warning in root layout | ~~**NEW HIGH bug (owner-reported, PM audit #79).**~~ **RESOLVED (Phase 170, PM audit #80).** Replaced `<Script>` with raw `<script dangerouslySetInnerHTML>` in `<head>`. Zero React warnings. | ✅ RESOLVED (Phase 170) | `src/app/layout.tsx` |
...
10. ~~Entire application must comply with **WCAG 2.2 AA** accessibility standards.~~ **COMPLETE (PM audit #80).** All sub-phases delivered including tablist arrow-key nav (108) and AvatarMenu keyboard nav (114). ToggleTheme hydration regression RESOLVED (Phase 169). `aria-checked` now matches visual state post-hydration.
...
19. ~~WCAG 2.2 AA compliance substantially delivered.~~ **PARTIAL (PM audit #79).** All sub-phases delivered: admin skip-link (72.1 ✅), opacity contrast (72.2 ✅), form labels/ARIA (72.3 ✅), table semantics (72.4 ✅), viewport zoom (101 ✅), AudioPlayer ARIA (99.5 ✅), WCAG E2E (97.1 ✅), landing contrast (103.1 ✅), plans contrast (103.2 ✅), heading order (103.3 ✅), duplicate landmarks (103.4 ✅), decorative icons aria-hidden (109 ✅), hamburger aria-expanded (110 ✅), **tablist arrow-key navigation (108 ✅)**, **AvatarMenu keyboard navigation (114 ✅)**. ToggleTheme hydration regression RESOLVED (Phase 169, PM audit #80). `aria-checked` now matches visual state.
...
24. Testing infrastructure rebuild COMPLETE through Phases 120.1–120.7 + 117 + 113.2 + 112.2 + 109 + 115 + 116. **TDD REBUILD DONE.** Previous 532-test suite deleted. Current: **101 suites, 597 tests — all pass. E2E: 8 spec files. Build passing. All 7 gates GREEN.** Phase 120.2 COMPLETE — all ~40 utility test files rebuilt. Phase 120.3 COMPLETE — all action tests rebuilt. Phase 120.4 COMPLETE — route tests rebuilt, openai split into 5 modules. Phase 120.5 COMPLETE — 24 component test files. Phase 120.6 COMPLETE — E2E rebuilt (5 specs, structural assertions). Phase 120.7 COMPLETE — thresholds raised to 85/80/85/85. Phase 134 COMPLETE — 3 new E2E specs (billing checkout, admin user ops, error boundary). Zero `as never` casts remaining.
25. Owner directives (March 2026 — FINAL): (a) **TDD rebuild — COMPLETE.** 597 tests (101 suites). 8 E2E specs. Zero `as never`. Coverage 85/80/85/85. Phases 120.1–120.7 ALL COMPLETE. (b) **Full admin configurability — PARTIAL.** FAQ (74.2) ✅, landing copy (104) ✅, stop msgs (107) ✅. **Remaining: promo/upgrade text hardcoded (Phase 162).** (c) **WCAG 2.2 AA compliance — FULLY COMPLETE.** Tabs (108) ✅, AvatarMenu (114) ✅. All other sub-directives COMPLETE: code reuse (TD-REUSE-01–05 resolved), data consumers (73.1+73.3), resource leaks (102), user cascades, server-side utilities, knip clean, shared Button (128.1+128.2), shared types (106).
...
45. ~~**CRITICAL production bug — Audio player "Controller is already closed" (Phase 168, PM audit #78).**~~ **RESOLVED (Phase 168 CODE-COMPLETE, verified PM audit #79, 2026-04-01).** All three paths implemented: (A) `controllerClosed` flag, (B) download Range support, (C) audio player lifecycle hardening. Archived to DONE.md.
46. ~~**NEW CRITICAL — ToggleTheme hydration mismatch (Phase 169, PM audit #79, 2026-04-01).**~~ **RESOLVED (Phase 169, PM audit #80).** `getInitialMode()` now always returns `"system"`. localStorage synced via `useEffect` after mount. WCAG 2.2 AA `aria-checked` regression resolved.
47. ~~**NEW HIGH — Script tag warning in root layout (Phase 170, PM audit #79, 2026-04-01).**~~ **RESOLVED (Phase 170, PM audit #80).** Replaced `<Script>` with raw `<script dangerouslySetInnerHTML>` in `<head>`. Zero React warnings.
48. **Phase 168 CODE-COMPLETE (verified PM audit #79, 2026-04-01).** All three fix paths (SSE controllerClosed guard, download Range support, audio player lifecycle hardening) confirmed implemented in codebase. Archived from TODO.md to DONE.md.
49. ~~**CRITICAL production bug — Stream STILL failing after deployment (Phase 160 RE-OPENED, PM audit #78).**~~ **MITIGATED (Phase 160.2, PM audit #80).** Proactive 55s timeout safety net implemented. Client receives graceful warning instead of raw error. Root cause persists: Vercel Hobby 60s function timeout. Full fix requires Vercel Pro upgrade or architecture change.
50. **CRITICAL production bug — Payment STILL failing after deployment (Phase 161 RE-OPENED, PM audit #78).** Code logic verified correct by triple-audit (Architect, Engineer, PM). Webhook signature verification, Zod `.strip()`, idempotency repair, error logging — all architecturally sound. Root cause: Stripe webhook endpoint in Dashboard misconfigured. Must verify: (1) endpoint URL matches production domain, (2) signing secret matches `STRIPE_WEBHOOK_SECRET` env var, (3) `checkout.session.completed` event type selected, (4) check Recent Deliveries for 400/500 responses. This is a zero-code ops fix.
...
**4 of 6 gates GREEN. Product Gate YELLOW, Admin Gate YELLOW, Validation Gate YELLOW.** DEPLOYED. Phases 169, 170, 160.2 COMPLETED (PM audit #80, 2026-04-01). 2 unit test failures remain. `cellesseon` rename pending.
...
| C    | Product Gate      | YELLOW | Stream error MITIGATED (Phase 160.2 — proactive 55s timeout). Root cause persists (Vercel Hobby 60s). Audio RESOLVED (168). Payment RESOLVED. Hydration RESOLVED (169). Script warning RESOLVED (170). |
| D    | Admin Gate        | YELLOW | Core admin-configurability delivered. **Remaining: promo/upgrade text still hardcoded (Phase 162). `cellesseon` legacy keys pending rename (Phase 172).**                                              |
| E    | Public Gate       | GREEN  | All 7 public routes accurate, legal content real, no obsolete trial messaging                                                                                                                          |
| F    | Validation Gate   | YELLOW | **2 unit test failures** (stale assertions — Phase 171). Lint (0/0), knip (0), TSC clean, E2E (8 specs, 49 tests), build passes, coverage 85/80/85/85. Zero `as never` casts.                          |

---

## 9. Current Execution Order

> Milestones 0–25 ALL COMPLETE. **DEPLOYED TO PRODUCTION.** Phases 169 (hydration fix), 170 (script tag fix), 160.2 (proactive timeout) ALL COMPLETED this session (PM audit #80). Phase 168 CODE-COMPLETE (audio fix — archived). BUG-PAYMENT RESOLVED. Phase 167 partially completed (targeted catches in 9 files). 34 parameterless catches remain repo-wide. 2 unit test failures (stale assertions). 5 src files still reference `cellesseon`.
> **3 of 6 gates GREEN (Product YELLOW, Admin YELLOW, Validation YELLOW).** E2E: 8 specs (49 tests). Zero `as never` casts.
...
**ACTIVE BUGS (PM audit #79, updated 2026-04-01):**

> 1. ~~**BUG-PAYMENT**~~: **RESOLVED.** Stripe webhook returning 200 OK. Payment test passed.
> 2. ~~**BUG-AUDIO**~~: **RESOLVED (Phase 168, archived).**
> 3. ~~**BUG-HYDRATION**~~: **RESOLVED (Phase 169, PM audit #80).** `aria-checked` now matches visual state post-hydration.
> 4. ~~**BUG-SCRIPT**~~: **RESOLVED (Phase 170, PM audit #80).** Zero React script tag warnings.
> 5. ~~**BUG-STREAM**~~: **MITIGATED (Phase 160.2, PM audit #80).** Proactive 55s timeout safety net sends graceful error. Root cause persists (Vercel Hobby 60s limit).
> 6. **TEST-FAIL-01 (NEW)**: `chat-body.test.tsx:160` — stale amber class assertions. **Phase 171.**
> 7. **TEST-FAIL-02 (NEW)**: `chat-sidebar-promo.test.tsx:19` — expects "Manage Plan", component renders "Upgrade Now". **Phase 171.**
> 8. **BRAND-LEGACY (NEW)**: 5 src files reference `cellesseon`. Owner directs rename to `droplet`. **Phase 172.**

**Priority order (PM audit #80, updated 2026-04-01 — test fixes + owner directives first, then hardening):**

1. **🔴 Phase 171 CRITICAL** — Fix 2 failing unit tests: (a) `chat-body.test.tsx:160` — update amber class assertions to match current component classes; (b) `chat-sidebar-promo.test.tsx:19` — change "Manage Plan" to "Upgrade Now".
2. **🔴 Phase 172 CRITICAL** — `cellesseon` → `droplet` localStorage key rename: (a) remove legacy key migration logic from `droplet-theme.tsx`, `layout.tsx`, `chat-sidebar-shell.tsx`; (b) update `privacy.json` and `cookies.json` references.
3. **Phase 167.2 HIGH** — Remaining 34 empty catch blocks across `src/`.
4. **Phase 162 HIGH** — Extract promo/upgrade text to admin-configurable settings (expanded scope: sidebar promo + plan promo + persona card + chat body).
5. **Phase 163 HIGH** — Add `global-error.tsx` for root layout error recovery.
6. **Phase 165 MEDIUM** — Checkout success page DB polling (safety net — payment working, deprioritized).
...
**Completed this session (PM audit #80) — archived to DONE.md:**

- Phase 169 — ToggleTheme hydration mismatch fix. **RESOLVED.**
- Phase 170 — Script tag warning fix. **RESOLVED.**
- Phase 160.2 — Proactive 55s stream timeout safety net. **RESOLVED.**

**Completed and deployed (Production):**

- Phase 160/160.1 — `maxDuration=60`, heartbeat, `didSendFinal`, stderr logging. **Deployed.**
- Phase 161 — Webhook idempotency. **VERIFIED IN PRODUCTION.**
- Phase 164 — Client timeout 70s. **Deployed.**
- Phase 166 — maxDuration on all 6 API routes. **Deployed.**
- Phase 167 (partial) — Targeted catch blocks in 9 files. **Deployed.** 34 remain.
...
| OI4  | WCAG 2.2 AA compliance                            | HIGH     | **COMPLETE (PM audit #80).** All sub-phases delivered. Tabs (108) ✅, AvatarMenu (114) ✅. ToggleTheme hydration mismatch RESOLVED (Phase 169). `aria-checked` now matches visual state.                                                                  |
...
| OI18 | ALL `cellesseon` references renamed to `droplet`  | CRITICAL | **OPEN.** 5 src files + 2 JSON files still reference `cellesseon` localStorage keys. Owner overrides deprecation protection. Phase 172.                                                                                                                   |
