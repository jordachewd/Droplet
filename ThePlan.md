...
1. **Audio playback error** — clicking PLAY on generated audio triggers `ERR_INVALID_STATE: Controller is already closed` (server-side SSE controller race + download route lacks HTTP Range support).
...
3. **Payment not registering** — Stripe payment succeeds but no Transaction created and User plan not updated. Code verified correct by triple-audit. Root cause: Stripe webhook endpoint misconfiguration (URL, signing secret, or event types).

The execution order is: **Fix Stripe webhook ops config (BUG 3) → Audio player + download Range support (Phase 168) → Proactive timeout safety net (Phase 160.2) → Checkout success polling (Phase 165) → Remaining catch blocks (Phase 167.2) → Admin promo text (Phase 162) → Global error boundary (Phase 163) → Env hardening (Phase 143) → Performance (Phase 144 config cache) → Remaining backlog.**
...
| Audio player controller error | **NEW CRITICAL production bug (PM audit #78, 2026-03-30).** Clicking PLAY on generated audio triggers `TypeError: Invalid state: Controller is already closed { code: 'ERR_INVALID_STATE' }`. Two contributing paths: (A) SSE stream controller race condition — heartbeat interval fires after `controller.close()` in finally block, trying `controller.enqueue()` on closed controller. Already caught by try/catch but produces uncaught server-side error. (B) Download route `/api/download` lacks HTTP Range request support (`Accept-Ranges: bytes`), essential for audio/video seek and replay. (C) Audio player component `previousAudioUrlRef` guard prevents re-initialization after cleanup/remount. **Phase 168 tracks fix.** | **🔴 NEW — investigation Phase 168.** | `src/components/shared/audio-player.tsx`, `src/app/api/download/route.tsx`, `src/app/api/openai/route.tsx` |
...
45. **CRITICAL production bug — Audio player "Controller is already closed" (Phase 168, PM audit #78).** NEW (2026-03-30). Two root causes: (A) SSE stream controller race condition — heartbeat interval fires after `controller.close()` in finally block, enqueue on closed controller throws. (B) Download route `/api/download` lacks HTTP Range request support, breaking audio element seek/replay. (C) Audio player `previousAudioUrlRef` guard prevents re-initialization after cleanup. Fix approach: add `controllerClosed` boolean flag before `controller.close()`; add Range request support to download route; harden audio player lifecycle.
...
47. **CRITICAL production bug — Payment STILL failing after deployment (Phase 161 RE-OPENED, PM audit #78).** Code logic verified correct by triple-audit (Architect, Engineer, PM). Webhook signature verification, Zod `.strip()`, idempotency repair, error logging — all architecturally sound. Root cause: Stripe webhook endpoint in Dashboard misconfigured. Must verify: (1) endpoint URL matches production domain, (2) signing secret matches `STRIPE_WEBHOOK_SECRET` env var, (3) `checkout.session.completed` event type selected, (4) check Recent Deliveries for 400/500 responses. This is a zero-code ops fix.
...
**4 of 6 gates GREEN. Product Gate RED, Admin Gate YELLOW.** DEPLOYED but 3 CRITICAL production bugs active (PM audit #78, 2026-03-30).
...
| C    | Product Gate      | RED    | 3 CRITICAL production bugs active: (1) Audio playback ERR_INVALID_STATE, (2) Stream error on media gen — Vercel 60s timeout, (3) Payment not registering — Stripe webhook misconfig. All deployed code is correct but insufficiently mitigated or ops-blocked. |
