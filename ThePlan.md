...
| Test infrastructure      | 65 unit test files organized by domain. Phase-suffixed duplicates consolidated. Failure path tests for media generation. Security injection tests. E2E specs consolidated. Error state E2E added. Server-only credential guards delivered (Phase 92). Extended server-only guards on 17 files (Phase 86). Admin table selection clearing tests (Phase 90.6). Admin bulk E2E, cleanup, behavioral tests (Phase 91.2–91.4). Phase 94.1–94.5 COMPLETE. Phase 95.1–95.4 COMPLETE. Phase 95-R COMPLETE (Chromium failures resolved — Playwright workers=1, resolvePlaywrightWorkers()). Phase 96.1 COMPLETE (conversation-stop merged). Phase 96.2 COMPLETE (pure utility un-mocking). Phase 96.3 COMPLETE (webhook idempotency/edge-case tests). Phase 99.1 COMPLETE (aria-live). Phase 99.3 COMPLETE (stable message keys via createMessageId). Phase 100.1–100.3 COMPLETE (isObjectRecord/VALID_PERSONA_ID_SET/legalReviewDisclaimer deduplication). **65 unit test suites, 400 tests (all pass). E2E: 108 passed, 0 failed, 25 skipped. All 6 gates GREEN. Coverage: 76.27/65.31/79.64/76.67 vs 76/65/79/76 (MET). Build passing.** Remaining: branch coverage 65% biggest gap, zero WCAG E2E (TD-WCAG-02), zero Zustand tests (TD-TEST-07), 1 remaining isObjectRecord duplicate (TD-REUSE-01 partial — Phase 100.4). NEW: viewport zoom WCAG critical (TD-WCAG-03 — Phase 101), AbortController leak (TD-LEAK-01 — Phase 102), AudioPlayer ARIA (TD-WCAG-04 — Phase 99.5). | ⚠️ Testing rebuild progressing (Phases 90–102). All gates GREEN. Coverage MET. WCAG major fixes delivered. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
19. WCAG 2.2 AA compliance substantially delivered: admin skip-link (72.1 ✅), opacity contrast (72.2 ✅), form labels/ARIA (72.3 ✅), table semantics (72.4 ✅), viewport zoom (101 ✅), AudioPlayer ARIA (99.5 ✅), WCAG E2E (97.1 ✅). Remaining: full audit pass for all routes.
...
24. Testing infrastructure rebuild delivered through Phase 100.3. Key phases: 90.1–90.7 (structure + tests), 91.1–91.5 (E2E), 92.1–92.2 (server-only guards), 86 (extended guards), 94.1–94.5 (config hardening), 95.1–95.4 (E2E structural assertions), 95-R (Chromium fix — Playwright workers=1 + resolvePlaywrightWorkers()), 96.1 (conversation-stop merge), 96.2 (pure utility un-mocking), 96.3 (webhook idempotency tests), 99.1 (aria-live), 99.3 (stable message keys), 100.1–100.3 (code deduplication — isObjectRecord, VALID_PERSONA_ID_SET, legalReviewDisclaimer). **65 suites, 400 tests — all GREEN. E2E: 108 passed, 0 failed, 25 skipped. Coverage gate MET at 76/65/79/76. All 6 gates GREEN. TD-WCAG-02 RESOLVED, TD-WCAG-03 RESOLVED, TD-WCAG-04 RESOLVED, TD-REUSE-01 FULLY RESOLVED, TD-LEAK-01 RESOLVED. Remaining: branch coverage 65% biggest gap, zero Zustand tests (TD-TEST-07).**
25. Owner directives (March 2026 update): TDD approach mandated. Full testing infrastructure rebuild from scratch authorized. **PM audit #42 triple-audit completed. PM audit #43–#45 verified. PM audit #46: Phases 101, 100.4, 102, 96.4, 99.5, 72.4, 97.1 ALL verified DONE. Remaining: 96.5–96.8, 97.2+, 98. New phases added: 101 (CRITICAL WCAG viewport), 100.4 (4th isObjectRecord), 102 (AbortController leak), 99.5 expanded (AudioPlayer ARIA + ESLint), 99.4 expanded (knip cleanup).**
26. Owner directives (March 2026 — expanded, PM audit #45): (a) TDD rebuild — refactor ALL unit and e2e tests from scratch. (b) Full admin configurability NON-NEGOTIABLE — NO HARDCODED data, everything admin-configurable. (c) WCAG 2.2 AA compliance — viewport RESOLVED (Phase 101). (d) `npm run knip` must stay clean — must stay clean. (e) `/app/profile` plan limitations display (Phase 66.1 delivered). (f) Code reuse maximized — TD-REUSE-01/02/03 ALL RESOLVED (Phase 100.1–100.4). (g) Client components must be pure data consumers. (h) Reduce resource leaks — AbortController added (Phase 102). TD-LEAK-01 RESOLVED. (i) User removal cascades Clerk + DB + all data (verified complete). (j) Components must consume data via props, not direct constant imports. (k) Entire codebase reduce unnecessary renders. (l) Admin plans/prices/features/settings/naming/descriptions/limits fully admin-configurable.
...
**Current status (post-Phase 102, PM audit #46):** Prettier, lint (0 errors, 10 warnings), tsc, unit tests (65 suites / 400 tests), E2E (108 passed, 0 failed, 25 skipped), and build all pass. **All 6 gates GREEN.** Coverage: 76.27/65.31/79.64/76.67 vs 76/65/79/76 (MET). TD-WCAG-03 RESOLVED (Phase 101), TD-REUSE-01 FULLY RESOLVED (Phase 100.4), TD-LEAK-01 RESOLVED (Phase 102), TD-WCAG-04 RESOLVED (Phase 99.5), TD-WCAG-02 RESOLVED (Phase 97.1).

---

## 9. Immediate Execution Order

Milestones 0–24 are COMPLETE. Milestone 25 IN PROGRESS. All Phases 1–102 complete (including 86, 90.6, 91.2–91.4, 92.1–92.2, 93.1–93.2, 94–96.4, 95-R, 97.1, 99.1, 99.3, 99.5, 100.1–100.4, 101, 102, 72.4). Remaining Milestone 25 phases: 96.5–96.8 (unit test depth), 97.2+ (E2E quality expansion), 98 (coverage gate). Remaining backlog: 99.2 (dialog), 99.4 (knip), 73.3 (admin data consumers), 74.2–74.3 (admin configurability), 87 (createTaskSchema strict).
...
| 8   | WCAG 2.2 AA accessibility compliance                         | HIGH     | ⚠️ MOSTLY COMPLETE. 72.1–72.4 ✅ Phase 101 ✅ 99.5 ✅ 97.1 ✅. Remaining: full audit pass.                                                                                                |
...
**Current priority (Milestone 25 — PM audit #42):**
...
| CRITICAL | 94        | Testing config & toolchain hardening (Vitest thresholds, Playwright, ESLint, tsconfig, factories)             |
| CRITICAL | 95        | Fix 4 failing E2E specs (structural assertions instead of hardcoded content)                                  |
| HIGH     | 96        | Unit test quality rebuild (merge duplicates, reduce mocks, add missing, Zustand tests)                        |
| HIGH     | 97        | E2E quality rebuild (WCAG via axe-core, admin propagation, Mongo dedup)                                       |
| HIGH     | 98        | Coverage gate achievement (branch 65→75%+, statement 76→80%+, raise thresholds)                               |
...
> **All Milestones 0–24 COMPLETE. All Phases 1–100.3 complete (including 86, 90.6, 91.2–91.4, 92.1–92.2, 93.1–93.2, 94–96.4, 95-R, 97.1, 99.1, 99.3, 99.5, 100.1–100.4, 101, 102, 72.4). 400 unit tests (65 suites) passing. 108 E2E passed, 0 failed, 25 skipped. All 6 gates GREEN. Coverage 76.27/65.31/79.64/76.67 vs 76/65/79/76 (MET). Build passing. Milestone 25 IN PROGRESS — testing infrastructure rebuild phases (96.5–98) + remaining quality fixes.**
...
| D5  | Refactor all unit and e2e tests from scratch; rebuild entire testing process   | CRITICAL | ⚠️ IN PROGRESS. Phases 88–96.4, 97.1 complete. Remaining: 96.5–96.8, 97.2+, 98.                                                                         |
...
| D9  | WCAG 2.2 AA accessibility compliance                                           | HIGH     | ⚠️ MOSTLY COMPLETE. 72.1–72.4 ✅. Phase 101 ✅ (viewport zoom). Phase 99.5 ✅ (AudioPlayer ARIA). Phase 97.1 ✅ (WCAG E2E). Remaining: full audit pass. |
...
| D14 | Clean unused via npm run knip                                                  | HIGH     | ⚠️ 3 unused exports remain (Phase 99.4 pending): createTestTransaction, createTestEntitlements, createMessageId.                                        |
...
| 3   | WCAG 2.2 AA accessibility compliance for entire app                            | HIGH     | ⚠️ MOSTLY COMPLETE. 72.1–72.4 ✅. Phase 101 ✅ (viewport). Phase 99.5 ✅ (AudioPlayer). Phase 97.1 ✅ (WCAG E2E). Remaining: full audit.                                                      |
...
| 8   | `npm run knip` — unused code audit workflow                                    | HIGH     | ⚠️ 3 unused exports (PM audit #45): createTestTransaction, createTestEntitlements, createMessageId. Phase 99.4.                                                                               |
...
| 12  | Full testing infrastructure rebuild — TDD approach, aggressive testing         | CRITICAL | ⚠️ IN PROGRESS. Phases 86–102 complete (including 95-R, 96.3–96.4, 97.1, 99.1, 99.3, 99.5, 100.1–100.4, 101, 102). All 6 gates GREEN. 65 suites / 400 tests. Remaining: 96.5–96.8, 97.2+, 98. |
...
24. **HIGH** — Phase 96: Unit test quality rebuild — **96.1–96.4 DONE**. Remaining: 96.5–96.8.
25. **HIGH** — Phase 97: E2E quality rebuild — **97.1 DONE** (WCAG via axe-core, 7 public routes). Remaining: admin propagation, Mongo dedup.
26. **HIGH** — Phase 98: Coverage gate achievement (branch 65→78%+, statement 76→82%+, raise thresholds).
27. **HIGH** — Phase 99: WCAG quick fixes + code quality. ~~99.1 DONE (aria-live), 99.3 DONE (stable keys), 99.5 DONE (AudioPlayer ARIA).~~ Remaining: 99.2 (native dialog), 99.4 (knip cleanup).
...
- ✅ 400 unit tests (65 suites). E2E 108/0/25. All 6 gates GREEN. Coverage MET.
...
- ⬜ Admin/upload/Zustand/component tests — Phase 96.5–96.8 PENDING.
- ✅ WCAG E2E tests via axe-core — Phase 97.1 DONE (7 public routes). TD-WCAG-02 RESOLVED.
- ⬜ Remaining E2E quality — Phase 97.2+ PENDING.
- ⬜ Coverage gate GREEN (branches ≥78%, statements ≥82%) — Phase 98 PENDING.
