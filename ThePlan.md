...
| Test infrastructure      | 64 unit test files organized by domain. Phase-suffixed duplicates consolidated. Failure path tests for media generation. Security injection tests. E2E specs consolidated. Error state E2E added. Server-only credential guards delivered (Phase 92). Extended server-only guards on 17 files (Phase 86). Admin table selection clearing tests (Phase 90.6). Admin bulk E2E, cleanup, behavioral tests (Phase 91.2–91.4). Phase 94.1–94.5 COMPLETE. Phase 95.1–95.4 COMPLETE. Phase 95-R COMPLETE (Chromium failures resolved — Playwright workers=1, resolvePlaywrightWorkers()). Phase 96.1 COMPLETE (conversation-stop merged). Phase 96.2 COMPLETE (pure utility un-mocking). Phase 96.3 COMPLETE (webhook idempotency/edge-case tests). Phase 99.1 COMPLETE (aria-live). Phase 99.3 COMPLETE (stable message keys via createMessageId). Phase 100.1–100.3 COMPLETE (isObjectRecord/VALID_PERSONA_ID_SET/legalReviewDisclaimer deduplication). **64 unit test suites, 393 tests (all pass). E2E: 87 passed, 0 failed, 25 skipped. All 6 gates GREEN. Coverage: 76.27/65.31/79.64/76.67 vs 76/65/79/76 (MET). Build passing.** Remaining: branch coverage 65% biggest gap, zero WCAG E2E (TD-WCAG-02), zero Zustand tests (TD-TEST-07), 1 remaining isObjectRecord duplicate (TD-REUSE-01 partial — Phase 100.4). NEW: viewport zoom WCAG critical (TD-WCAG-03 — Phase 101), AbortController leak (TD-LEAK-01 — Phase 102), AudioPlayer ARIA (TD-WCAG-04 — Phase 99.5). | ⚠️ Testing rebuild progressing (Phases 90–100.3). All gates GREEN. Coverage MET. WCAG fixes needed. | tests/unit/**, tests/e2e/**, vitest.config.mts, playwright.config.ts                            |
...
24. Testing infrastructure rebuild delivered through Phase 100.3. Key phases: 90.1–90.7 (structure + tests), 91.1–91.5 (E2E), 92.1–92.2 (server-only guards), 86 (extended guards), 94.1–94.5 (config hardening), 95.1–95.4 (E2E structural assertions), 95-R (Chromium fix — Playwright workers=1 + resolvePlaywrightWorkers()), 96.1 (conversation-stop merge), 96.2 (pure utility un-mocking), 96.3 (webhook idempotency tests), 99.1 (aria-live), 99.3 (stable message keys), 100.1–100.3 (code deduplication — isObjectRecord, VALID_PERSONA_ID_SET, legalReviewDisclaimer). **64 suites, 393 tests — all GREEN. E2E: 87 passed, 0 failed, 25 skipped. Coverage gate MET at 76/65/79/76. All 6 gates GREEN. Remaining: branch coverage 65% biggest gap, zero WCAG E2E (TD-WCAG-02), zero Zustand tests (TD-TEST-07), 1 isObjectRecord duplicate (effective-model-config.ts — Phase 100.4), viewport zoom WCAG critical (Phase 101), AbortController leak (Phase 102).**
25. Owner directives (March 2026 update): TDD approach mandated. Full testing infrastructure rebuild from scratch authorized. **PM audit #42 triple-audit completed. PM audit #43: Phases 94–96.1 verified DONE. PM audit #44: Phase 96.2 verified DONE. PM audit #45: Phases 95-R, 96.3, 99.1, 99.3, 100.1–100.3 verified DONE. Remaining: 96.4–96.8, 97, 98. New phases added: 101 (CRITICAL WCAG viewport), 100.4 (4th isObjectRecord), 102 (AbortController leak), 99.5 expanded (AudioPlayer ARIA + ESLint), 99.4 expanded (knip cleanup).**
26. Owner directives (March 2026 — expanded, PM audit #45): (a) TDD rebuild — old test flows rebuilt, not patched. (b) Full admin configurability NON-NEGOTIABLE. (c) WCAG 2.2 AA compliance is a release blocker — **CRITICAL: viewport zoom restriction found (Phase 101)**. (d) `npm run knip` must stay clean — 3 unused exports detected (Phase 99.4). (e) `/app/profile` plan limitations display (Phase 66.1 delivered). (f) Code reuse maximized — TD-REUSE-02/03 RESOLVED, TD-REUSE-01 partial (1 remaining in effective-model-config.ts — Phase 100.4). (g) Client components must be pure data consumers. (h) Reduce resource leaks — **AbortController missing on chat streaming fetch (Phase 102)**. (i) User removal cascades Clerk + DB + all data (verified complete). (j) Components must consume data via props, not direct constant imports.
...
**Current status (post-Phase 100.3, PM audit #45):** Prettier, lint (0 errors, 11 warnings), tsc, unit tests (64 suites / 393 tests), E2E (87 passed, 0 failed, 25 skipped), and build all pass. **All 6 gates GREEN.** Coverage: 76.27/65.31/79.64/76.67 vs 76/65/79/76 (MET). CRITICAL: viewport zoom WCAG violation (Phase 101). HIGH: 4th isObjectRecord duplicate (Phase 100.4), AbortController leak (Phase 102), AudioPlayer ARIA (Phase 99.5).

---

## 9. Immediate Execution Order

Milestones 0–24 are COMPLETE. Milestone 25 IN PROGRESS. All Phases 1–100.3 complete (including 86, 90.6, 91.2–91.4, 92.1–92.2, 93.1–93.2, 94–96.3, 95-R, 99.1, 99.3, 100.1–100.3). Remaining Milestone 25 phases: 96.4–96.8 (unit test depth), 97 (E2E quality), 98 (coverage gate). New phases: 101 (CRITICAL viewport zoom), 100.4 (isObjectRecord), 102 (AbortController). Remaining backlog: 99.2 (dialog), 99.4 (knip), 99.5 (AudioPlayer ARIA), 72.4 (WCAG tables), 73.3 (admin data consumers), 74.2–74.3 (admin configurability), 87 (createTaskSchema strict).
...
> **All Milestones 0–24 COMPLETE. All Phases 1–100.3 complete (including 86, 90.6, 91.2–91.4, 92.1–92.2, 93.1–93.2, 94–96.3, 95-R, 99.1, 99.3, 100.1–100.3). 393 unit tests (64 suites) passing. 87 E2E passed, 0 failed, 25 skipped. All 6 gates GREEN. Coverage 76.27/65.31/79.64/76.67 vs 76/65/79/76 (MET). Build passing. Milestone 25 IN PROGRESS — testing infrastructure rebuild phases (96.4–98) + quality fixes (99, 100.4, 101, 102).**
...
| D4  | DEEP analysis of entire e2e testing flow — AGGRESSIVE testing process          | CRITICAL | ✅ E2E FIXED (Phase 95-R). 87 passed, 0 failed, 25 skipped. Remaining: Phase 97 (WCAG axe-core E2E).                                                                        |
| D5  | Refactor all unit and e2e tests from scratch; rebuild entire testing process   | CRITICAL | ⚠️ IN PROGRESS. Phases 88–93 complete. New phases 94–98 defined by PM audit #42.                                                                                            |
...
| D8  | RE-USE repetitive code as much as possible                                     | HIGH     | ✅ MOSTLY DONE. Phase 100.1–100.3 resolved 3 patterns. TD-REUSE-02/03 RESOLVED. TD-REUSE-01 partial (1 remaining — Phase 100.4).                                            |
| D9  | WCAG 2.2 AA accessibility compliance                                           | HIGH     | ⚠️ CRITICAL ISSUE FOUND. 72.1–72.3 ✅. Remaining: 72.4 (table semantics), **Phase 101 (viewport zoom restriction — WCAG SC 1.4.4 FAILURE)**, Phase 99.5 (AudioPlayer ARIA). |
| D10 | Components must be data consumers                                              | CRITICAL | ✅ COMPLETE (Phase 73.1). 3 admin components remain (73.3).                                                                                                                 |
| D11 | Good practices — reduce unnecessary renders and resource leaks                 | CRITICAL | ⚠️ ISSUE FOUND. Most useEffect cleanups verified. **Chat streaming fetch lacks AbortController — HTTP connection leak (Phase 102).**                                        |
...
| D14 | Clean unused via npm run knip                                                  | HIGH     | ⚠️ 3 unused exports found (PM audit #45): createTestTransaction, createTestEntitlements, createMessageId. Phase 99.4.                                                       |
...
| 2   | RE-USE repetitive code as much as possible                                     | HIGH     | ✅ MOSTLY DONE. Phase 100.1–100.3 resolved 3 patterns. TD-REUSE-02/03 RESOLVED. 1 remaining (Phase 100.4).                               |
| 3   | WCAG 2.2 AA accessibility compliance for entire app                            | HIGH     | ⚠️ CRITICAL ISSUE. 72.1–72.3 ✅. **Phase 101: viewport zoom (CRITICAL WCAG failure).** Remaining: 72.4, 99.5.                            |
| 4   | Components must be data consumers (especially `"use client"`)                  | CRITICAL | ✅ COMPLETE (Phase 73.1). 5 core components converted.                                                                                   |
| 5   | Good practices — reduce unnecessary renders and resource leaks                 | CRITICAL | ⚠️ ISSUE. Chat streaming fetch lacks AbortController (Phase 102). Most cleanups verified.                                                |
...
| 12  | Full testing infrastructure rebuild — TDD approach, aggressive testing         | CRITICAL | ⚠️ IN PROGRESS. Phases 86–100.3 complete (including 95-R, 96.3, 99.1, 99.3). All 6 gates GREEN. 393 tests. Remaining: 96.4–96.8, 97, 98. |
...
| 14  | RE-USE repetitive code — extract duplicated patterns                           | HIGH     | ✅ MOSTLY DONE. TD-REUSE-02/03 RESOLVED (Phase 100.2/100.3). TD-REUSE-01 partial (1 remaining — Phase 100.4).                            |
...
24. **HIGH** — Phase 96: Unit test quality rebuild — **96.1–96.3 DONE**. Remaining: 96.4–96.8.
25. **HIGH** — Phase 97: E2E quality rebuild (WCAG via axe-core, admin propagation, Mongo dedup).
26. **HIGH** — Phase 98: Coverage gate achievement (branch 65→78%+, statement 76→82%+, raise thresholds).
27. **HIGH** — Phase 99: WCAG quick fixes + code quality. ~~99.1 DONE (aria-live), 99.3 DONE (stable keys).~~ Remaining: 99.2 (native dialog), 99.4 (knip cleanup), 99.5 (AudioPlayer ARIA — expanded PM audit #45).
28. **HIGH** — Phase 100: Code deduplication. ~~100.1–100.3 DONE.~~ Remaining: 100.4 (4th isObjectRecord in effective-model-config.ts).
29. **CRITICAL** — Phase 101: Remove viewport zoom restriction — WCAG 2.2 AA SC 1.4.4 failure. **NEW — PM audit #45.**
30. **HIGH** — Phase 102: Add AbortController to chat streaming fetch — resource leak fix. **NEW — PM audit #45.**
...
- ✅ 393 unit tests (64 suites). E2E 87/0/25. All 6 gates GREEN. Coverage MET.
- ⬜ Viewport zoom WCAG fix — Phase 101 **CRITICAL**.
- ⬜ 4th isObjectRecord dedup — Phase 100.4 PENDING.
- ⬜ AbortController resource leak — Phase 102 PENDING.
- ⬜ AudioPlayer ARIA + ESLint — Phase 99.5 PENDING.
- ⬜ Admin/upload/Zustand/component tests added — Phase 96.4–96.8 PENDING.
- ⬜ WCAG E2E tests via axe-core — Phase 97 PENDING.
