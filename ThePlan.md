...
47. **CRITICAL production bug — Payment STILL failing (RE-OPENED 3rd time, 2026-03-31).** Owner now confirms: Stripe webhook returns HTTP 200 OK for ALL requests AND payment test succeeded. **PM audit #81 ROOT CAUSE HYPOTHESIS (code-level investigation):** Webhook handler code logic is verified CORRECT. All event type matching, Zod schemas, Transaction creation, User plan update, idempotency guards, and error handling are sound. Most likely cause: `checkout.session.completed` event type is NOT selected/enabled in the Stripe Dashboard webhook configuration. Non-checkout events (payment_intent.succeeded, charge.succeeded) DO reach the webhook and return 200 "Unhandled event" — making owner see 200 OK. **Action plan:** (1) Deploy Phase 169 diagnostic logging to confirm which event types are received, (2) Owner verify Stripe Dashboard → Webhooks → Events → ensure checkout.session.completed is selected.
48. ~~**CRITICAL — Hydration mismatch (Phase 170, PM audit #80).**~~ **ALREADY RESOLVED (PM audit #81 verified, 2026-03-31).** Code already uses `useState<ThemeMode>("system")` with `useEffect` for localStorage read. No `getInitialMode` function exists. TODO.md was stale.
49. ~~**CRITICAL — Script tag error (Phase 171, PM audit #80).**~~ **ALREADY RESOLVED (PM audit #81 verified, 2026-03-31).** `layout.tsx` already uses `<Script src="/scripts/theme-init.js">`. File exists at `public/scripts/theme-init.js` with correct IIFE. TODO.md was stale.
50. ~~**CRITICAL — Test regression Phase 168.1 (PM audit #79).**~~ **ALREADY RESOLVED (PM audit #81 verified, 2026-03-31).** 597 tests pass (101 suites). chat-sidebar-promo test is fixed. TODO.md was stale.
...
> Milestones 0–25 ALL COMPLETE. **DEPLOYED TO PRODUCTION (confirmed 2026-03-31).** 2 CRITICAL production bugs active (streaming, payment). BUG-PAYMENT RE-OPENED (2026-03-31 — owner reports webhook 200 OK but no Transaction created, no User plan updated; code investigation required). Phase 168 COMPLETE (audio player fix deployed). Phase 170/171 (hydration + script fix) prioritized ahead of bug investigation. Phase 167 partially completed (targeted catches in 9 files). 35 parameterless catches remain repo-wide. Phase 160.2 pending (proactive timeout safety net).
...
**ADMIN CONFIGURABILITY NOTE (2026-03-31):** Owner reiterates that full admin configurability (OI2/OI10/OI17) is HIGH priority. All plans, prices, features, settings, naming, descriptions, limits, and text copy must be fully configurable from `/admin/settings` — NO hardcoded references in code. **Deferred until CRITICAL production bugs (BUG-PAYMENT, BUG-STREAM) are resolved.**

**Priority order (updated 2026-03-31, PM audit #80 — Phase 170/171 prioritized, BUG-PAYMENT re-opened 3rd time, Phase 168 complete):**

1. **Phase 170 HIGH** — Hydration mismatch fix: theme toggle `aria-checked` and tooltip text differ between server and client for dark-mode users. Causes full hydration recovery re-render on every page load.
2. **Phase 171 HIGH** — Script tag extraction: `<Script>` with inline children in RootLayout triggers React 19 error in Next.js 16.2.1. Theme init script may silently fail.
3. **🔴 Phase 169 CRITICAL** — Stripe webhook diagnostic logging (BUG-PAYMENT investigation). Add event type logging + success path logging to webhook handler. Deploy. Have owner test payment and check Vercel function logs to identify which event types are reaching the endpoint and which code path returns 200.
4. **🔴 Phase 160.2 CRITICAL** — Proactive timeout safety net: set a maxDuration - 5s timer that sends graceful {type: "error", error: "Media generation timed out. Your content may still be processing."} before Vercel kills the function. Also discuss Vercel Pro upgrade ($20/mo, 300s timeout) with owner.
5. **🔴 Phase 168.1 CRITICAL** — Fix `chat-sidebar-promo.test.tsx` test regression. Validation Gate blocker.
6. **Phase 167.2 HIGH** — Remaining 35 empty catch blocks across `src/`.
7. **Phase 162 HIGH** — Extract promo/upgrade text to admin-configurable settings.
8. **Phase 163 HIGH** — Add `global-error.tsx` for root layout error recovery.
9. **Phase 165 MEDIUM** — Checkout success page DB polling.
10. **Phase 143 MEDIUM** — Replace `as string` / `!` casts on env vars with runtime validation.
11. **Phase 144 MEDIUM** — Admin config in-memory cache with 30s TTL.
12. **Phase 145 MEDIUM** — Upload filename collision fix (`crypto.randomUUID()`).
13. **Phase 146 LOW** — Admin user detail transaction `.limit(50)`.
14. **Phase 147 LOW** — Rename `.tsx` utility files to `.ts` where no JSX.
15. **Phase 148 LOW** — Admin bulk operations partial-failure reporting.
...
| OI17 | Plans/prices/features configurable from admin     | HIGH     | **PARTIAL — owner re-emphasized HIGH priority (2026-03-31).** Core pricing, limits, models, personas, theme, currency, support email admin-configurable. Stop msgs (107) ✅. FAQ (74.2) ✅. Landing/hero/about (104) ✅. **Remaining: promo/upgrade text hardcoded (Phase 162).** Deferred until CRITICAL bugs resolved. OI2/OI10 overlap. |
