...
## Phase 207 — Upload Magic Byte Validation (Security) — COMPLETED (2026-04-05)

> Engineer delivered (PM audit #94). Added magic byte signature validation for JPEG (`FF D8 FF`), PNG (`89 50 4E 47 0D 0A 1A 0A`), GIF (`47 49 46 38`), WebP (`RIFF`+`WEBP`). `validateImageMagicBytes()` and `doesImageMagicBytesMatchMimeType()` exported from `upload-file-validation.ts`. Enforced in upload route before S3 write — returns 400 on mismatch. 11 new unit tests.

- [x] Magic byte signatures for all 4 supported image formats
- [x] Cross-validation: detected format must match declared MIME type
- [x] Upload route rejects mismatched or unknown content before S3 write
- [x] 11 unit tests covering all formats, mismatches, and non-image rejection
- [x] Build passes, tests pass

---

## Phase 206 — Upload Error Message Propagation + Client-Side Validation — COMPLETED (2026-04-05)

> Engineer delivered (PM audit #94). Three fixes: (1) catch block uses `error.message` instead of generic text, (2) `accept` attribute narrowed from `image/*` to specific MIME types, (3) client-side `validateSelectedImageFile()` pre-validates before upload. Double validation in `handleImageChange` and `handleSubmit`.

- [x] Server error messages propagated to UI
- [x] File picker restricted to JPEG/PNG/WebP/GIF
- [x] `.avif` and unsupported types rejected with specific error before upload
- [x] Build passes, tests pass

---

## Phase 205 — Sidebar Live Update on New Chat — COMPLETED (2026-04-05)

> Engineer delivered (PM audit #94). `useRouter()` + `router.refresh()` called after new task creation in `ChatWrapper`. Ref-based one-time guard (`hasRefreshedSidebarForCurrentTaskRef`) prevents refresh on every message — only fires on first message of new conversation. Guard resets when `dbTaskId` returns to `null`. Called from all 5 response-handling paths. 2 new unit tests.

- [x] New conversation appears in sidebar after first message without browser refresh
- [x] Subsequent messages do not trigger unnecessary refreshes
- [x] Guard resets correctly on new conversation
- [x] 2 new unit tests (refresh on new, no refresh on existing)
- [x] Build passes, tests pass

---

## Phase 204 — Set All API Route Timeouts to Maximum — COMPLETED (2026-04-05)

> Engineer delivered (PM audit #94). All 4 API routes changed from `maxDuration = 30` to `maxDuration = 60`. All 6 API routes now at Vercel Hobby ceiling (60s). Files: `stripe/route.tsx`, `upload/route.tsx`, `download/route.tsx`, `aws/route.tsx`.

- [x] All 6 API routes at `maxDuration = 60`
- [x] Build passes
